...
- Top-level workspace members: `crates/vesc-rust-poc` (device-side `no_std` crate), `crates/vesc-cli`, `crates/vesc-pkg`, `crates/vesc-protocol`.
- Package work centers on the BLE loopback test package; packaging logic lives in `crates/vesc-pkg` and the final artifact path is rooted under `target/vescpkg`.
