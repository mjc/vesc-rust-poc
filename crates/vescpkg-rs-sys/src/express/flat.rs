...
impl<'a> ExpressFlatValue<'a> {
    ...
    pub fn push_cons(&mut self) -> Result<(), ExpressFlatValueError> {
        self.ensure_open()?;
        let push: FCons = unsafe { self.interface.function(ExpressSlot::FCons) }
            .map_err(ExpressFlatValueError::Unavailable)?;
        accepted(unsafe { push(&mut self.raw) })
    }

    /// Append a symbol identifier.
    pub fn push_symbol(&mut self, symbol: u32) -> Result<(), ExpressFlatValueError> {
        self.ensure_open()?;
        let push: FSym = unsafe { self.interface.function(ExpressSlot::FSym) }
            .map_err(ExpressFlatValueError::Unavailable)?;
        accepted(unsafe { push(&mut self.raw, symbol) })
    }

    /// Append an immediate LispBM integer.
    pub fn push_i(&mut self, value: i32) -> Result<(), ExpressFlatValueError> {
        self.ensure_open()?;
        let push: FI = unsafe { self.interface.function(ExpressSlot::FI) }
            .map_err(ExpressFlatValueError::Unavailable)?;
        accepted(unsafe { push(&mut self.raw, value) })
    }

    /// Append a byte value.
    pub fn push_byte(&mut self, value: u8) -> Result<(), ExpressFlatValueError> {
        self.ensure_open()?;
    ...
    pub fn push_i32(&mut self, value: i32) -> Result<(), ExpressFlatValueError> {
        self.ensure_open()?;
    ...
    pub fn push_u32(&mut self, value: u32) -> Result<(), ExpressFlatValueError> {
        self.ensure_open()?;
    ...
    /// Append an `f32` value.
    pub fn push_float(&mut self, value: f32) -> Result<(), ExpressFlatValueError> {
        self.ensure_open()?;
    ...
    pub fn push_i64(&mut self, value: i64) -> Result<(), ExpressFlatValueError> {
        self.ensure_open()?;
    ...
    pub fn push_u64(&mut self, value: u64) -> Result<(), ExpressFlatValueError> {
        self.ensure_open()?;
    ...
    pub fn push_byte_array(&mut self, bytes: &[u8]) -> Result<(), ExpressFlatValueError> {
        self.ensure_open()?;
        ...
        let push: FLbmArray = unsafe { self.interface.function(ExpressSlot::FLbmArray) }
            .map_err(ExpressFlatValueError::Unavailable)?;
        accepted(unsafe { push(&mut self.raw, count, bytes.as_ptr().cast_mut()) })
    }

    /// Finish this value before transferring it to LispBM.
    pub fn finish(&mut self) -> Result<(), ExpressFlatValueError> {
        if self.finished {
            return Ok(());
        }
        let finish: LbmFinishFlatten =
            unsafe { self.interface.function(ExpressSlot::LbmFinishFlatten) }
                .map_err(ExpressFlatValueError::Unavailable)?;
        accepted(unsafe { finish(&mut self.raw) })?;
        self.finished = true;
        Ok(())
    }
...
fn accepted(result: bool) -> Result<(), ExpressFlatValueError> {
    result.then_some(()).ok_or(ExpressFlatValueError::Rejected)
}
...
#[cfg(test)]
mod tests {
    use super::{ExpressFlatValue, ExpressFlatValueError};
    use crate::express::ExpressInterface;

    fn finished_value() -> ExpressFlatValue<'static> {
        static WORDS: [u32; 1] = [1];
        let interface = ExpressInterface::from_words(&WORDS).expect("version");
        ExpressFlatValue {
            interface,
            raw: super::super::types::ExpressFlatValueRaw::empty(),
            finished: true,
        }
    }

    #[test]
    fn finished_values_reject_appends_and_finish_is_idempotent() {
        let mut value = finished_value();
        assert_eq!(value.push_i(1), Err(ExpressFlatValueError::Rejected));
