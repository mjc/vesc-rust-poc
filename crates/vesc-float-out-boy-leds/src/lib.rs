...
macro_rules! assert_f32_eq {
    ($actual:expr, $expected:expr $(,)?) => {{
        ...
        assert!(
            (!actual.is_nan() && actual.to_bits() == expected.to_bits())
                || (actual.is_finite()
                    && expected.is_finite()
...
/// Compatibility namespace for the former in-package LCM LED types.
pub mod lcm {
...
/// Runtime power state for a Float Out Boy LED surface.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FloatOutBoyLedPower {
    /// Surface is off.
    Off,
    /// Surface is on.
    On,
...
impl FloatOutBoyLedPower {
    #[must_use]
    /// Return whether the surface is powered.
    pub const fn is_on(self) -> bool {
        matches!(self, Self::On)
    }

    #[must_use]
    /// Convert a Boolean enable flag into a power state.
...
/// Runtime LED power overrides owned by package state.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl FloatOutBoyLedRuntimeStatus {
    #[must_use]
    /// Build runtime status from the two LED power states.
    pub const fn new(power: FloatOutBoyLedPower, headlights_power: FloatOutBoyLedPower) -> Self {
        Self {
            power,
            headlights_power,
        }
    ...
    #[must_use]
    /// Return whether the LED surface is enabled.
    pub const fn is_enabled(self) -> bool {
        self.power.is_on()
    }

    #[must_use]
    /// Return whether the headlights are enabled.
    pub const fn are_headlights_on(self) -> bool {
        self.headlights_power.is_on()
    }

    #[must_use]
    /// Apply runtime power overrides to a static LED config.
    pub const fn apply(self, mut config: FloatOutBoyLedsConfig) -> FloatOutBoyLedsConfig {
...
impl FloatOutBoyLedRenderer {
    ...
    /// Return the confirmation animation start time for package-level tests.
    #[must_use]
