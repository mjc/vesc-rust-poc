...
//!
//! This module supports the source-backed VESC/Refloat outputs on B6, B7, and
//! C9. C6 and C7 are retained as additional STM32F4 timer/DMA mappings for
//! boards that explicitly wire those pads; they are not outputs declared by
//! the current `vesc_pkg` or Refloat package source and must not be selected
//! without board-level verification. Each pin resolves to its complete GPIO,
...
pub enum OutputPin {
    ...
    B7,
    /// Additional STM32 pin C6, TIM3 channel 1, DMA1 stream 4/channel 5.
    ///
    /// This mapping is not declared by the current VESC package source.
    C6,
    /// Additional STM32 pin C7, TIM3 channel 2, DMA1 stream 5/channel 5.
    ///
    /// This mapping is not declared by the current VESC package source.
    C7,
...
mod tests {
    ...
    #[test]
    fn source_and_additional_stm32_pin_maps_keep_their_timer_dma_tuples() {
