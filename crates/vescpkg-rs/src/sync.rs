...
impl FirmwareMutex {
    ...
    pub fn new() -> Option<Self> {
        NonNull::new(call_vesc_ffi!(vesc_mutex_create())).map(|handle| Self { handle })
    }

    /// Lock this mutex and return a guard that unlocks it on every drop path.
    #[must_use]
    pub fn lock(&self) -> FirmwareMutexGuard<'_> {
        call_vesc_ffi!(vesc_mutex_lock(self.handle.as_ptr()));
...
impl Drop for FirmwareMutex {
    fn drop(&mut self) {
        call_vesc_ffi!(vesc_free(self.handle.as_ptr()));
    }
}
...
impl Drop for FirmwareMutexGuard<'_> {
    fn drop(&mut self) {
        call_vesc_ffi!(vesc_mutex_unlock(self.mutex.handle.as_ptr()));
    }
}
...
impl FirmwareSemaphore {
    ...
    #[must_use]
    pub fn new() -> Option<Self> {
        NonNull::new(call_vesc_ffi!(vesc_sem_create())).map(|handle| Self { handle })
    }

    /// Block until the semaphore is signaled.
    pub fn wait(&self) {
        call_vesc_ffi!(vesc_sem_wait(self.handle.as_ptr()));
    }
    ...
    pub fn signal(&self) {
        call_vesc_ffi!(vesc_sem_signal(self.handle.as_ptr()));
    }

    /// Reset the semaphore to its unsignaled state.
    pub fn reset(&self) {
        call_vesc_ffi!(vesc_sem_reset(self.handle.as_ptr()));
    }
}
