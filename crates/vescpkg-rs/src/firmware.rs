...
/// Owned response bytes produced by an app-data callback.
///
/// The callback trampoline sends these bytes after releasing package state, so
/// interface I/O cannot hold the package-state lock.
...
impl AppDataResponse {
    ...
    /// # Errors
    ///
    /// Returns [`crate::AppDataSendError::PayloadTooLarge`] when `bytes`
    /// exceeds VESC's 511-byte app-data limit.
    pub fn write(&mut self, bytes: &[u8]) -> Result<(), crate::AppDataSendError> {
        if bytes.len() > MAX_APP_DATA_PAYLOAD_LEN {
            return Err(crate::AppDataSendError::PayloadTooLarge);
        }
        self.bytes[..bytes.len()].copy_from_slice(bytes);
        self.len = u16::try_from(bytes.len()).ok();
        Ok(())
    }

    /// Borrow the pending response, if the callback produced one.
    #[must_use]
...
impl Default for AppDataResponse {
    fn default() -> Self {
        Self::new()
    }
}
...
pub trait AppDataHandler {
    ...
    fn handle(state: &mut Self::State, packet: AppDataPacket<'_>, response: &mut AppDataResponse);
}
...
#[cfg(any(test, all(not(feature = "test-support"), target_arch = "arm")))]
fn dispatch_app_data_response<T: AppDataHandler>(
    source: crate::PackageStateAccess<'_, T::State>,
    packet: AppDataPacket<'_>,
    response: &mut AppDataResponse,
    send: impl FnOnce(&[u8]),
) -> bool {
    response.clear();
    let handled = source
        .with_mut(|state| T::handle(state, packet, response))
        .is_some();
    if handled && let Some(bytes) = response.as_bytes() {
        send(bytes);
    }
    handled
}
...
pub unsafe extern "C" fn app_data_handler<T: PackageAppDataCallback>(data: *mut u8, len: u32) {
    ...
    let _ = source.with_app_data_response(|response| {
        let _ = dispatch_app_data_response::<T>(source, packet, response, |bytes| {
            let _ = crate::Firmware::new().app_data().send(bytes);
        });
    });
}
...
mod tests {
    ...
    impl AppDataHandler for AppDataDispatchHandler {
        ...
        fn handle(
            ...
            state.handled = true;
            let _ = response.write(packet.as_bytes());
        }
    }
    ...
    #[test]
    fn app_data_response_owns_callback_local_bytes() {
        let mut response = AppDataResponse::new();
        let mut callback_local = [101, 31, 4, 2];

        assert_eq!(response.write(&callback_local), Ok(()));
        callback_local.fill(0);

        assert_eq!(response.as_bytes(), Some(&[101, 31, 4, 2][..]));
    }

    #[test]
    fn app_data_response_enforces_the_vesc_payload_limit() {
        let mut response = AppDataResponse::new();

        assert_eq!(response.write(&[0; 511]), Ok(()));
        assert_eq!(
            response.write(&[0; 512]),
            Err(crate::AppDataSendError::PayloadTooLarge)
        );
    }

    #[test]
    fn app_data_response_send_runs_after_package_state_is_released() {
        ...
        assert!(dispatch_app_data_response::<AppDataDispatchHandler>(
            source,
            packet,
            &mut response,
            |bytes| {
