...
pub trait AppDataHandler {
    ...
    fn handle(state: &mut Self::State, packet: AppDataPacket<'_>, reply: &mut AppDataReply<'_>);
}
...
fn prepare_app_data_reply<'a, T: AppDataHandler>(
    ...
    buffer.clear();
    source.with_mut(|state| {
        let mut reply = AppDataReply::new(buffer);
        T::handle(state, packet, &mut reply);
    })?;
...
mod tests {
    ...
    fn prepared_app_data_reply_sends_after_package_state_is_released() {
        let mut state = AppDataDispatchState { handled: false };
        ...
        reply.send_with(|bytes| {
            ...
            assert_eq!(
                APP_DATA_DISPATCH_STATE.with(|state| state.handled),
                Some(true)
            );
