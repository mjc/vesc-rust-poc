...
/// Maximum app-data payload accepted by VESC firmware.
pub const MAX_APP_DATA_PAYLOAD_LEN: usize = 511;

/// Failure returned when an app-data callback cannot stage its reply.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub enum AppDataReplyError {
    /// The reply exceeds the firmware's 511-byte app-data limit.
    PayloadTooLarge,
}

impl core::fmt::Display for AppDataReplyError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::PayloadTooLarge => f.write_str("app-data reply exceeds 511 bytes"),
        }
    }
}

impl core::error::Error for AppDataReplyError {}
...
/// Runtime-owned storage for one app-data reply.
pub(crate) struct AppDataReplyBuffer {
    bytes: [u8; MAX_APP_DATA_PAYLOAD_LEN],
    len: Option<u16>,
}
...
impl AppDataReplyBuffer {
    ...
    pub(crate) const fn new() -> Self {
        Self {
            bytes: [0; MAX_APP_DATA_PAYLOAD_LEN],
            len: None,
        }
    }
    ...
    fn as_bytes(&self) -> Option<&[u8]> {
        self.len.and_then(|len| self.bytes.get(..usize::from(len)))
    }
}

/// Callback-scoped writer for one app-data reply.
///
/// Package code can stage reply bytes through this capability but cannot
/// construct or retain the runtime-owned storage behind it.
pub struct AppDataReply<'a> {
    buffer: &'a mut AppDataReplyBuffer,
}
...
impl AppDataReply<'_> {
    ...
    /// Copy one callback-local response into owned storage.
    ///
    /// # Errors
    ///
    /// Returns [`AppDataReplyError::PayloadTooLarge`] when `bytes`
    /// exceeds VESC's 511-byte app-data limit.
    pub fn write(&mut self, bytes: &[u8]) -> Result<(), AppDataReplyError> {
        if bytes.len() > MAX_APP_DATA_PAYLOAD_LEN {
            return Err(AppDataReplyError::PayloadTooLarge);
        }
        let len = u16::try_from(bytes.len()).map_err(|_| AppDataReplyError::PayloadTooLarge)?;
...
#[cfg(any(test, all(not(feature = "test-support"), target_arch = "arm")))]
struct PreparedAppDataReply<'a>(&'a [u8]);

#[cfg(any(test, all(not(feature = "test-support"), target_arch = "arm")))]
impl PreparedAppDataReply<'_> {
    fn send_with(self, send: impl FnOnce(&[u8])) {
        send(self.0);
    }
}
...
impl<'a, T: crate::PackageRuntimeState> StatefulCallbackContext<'a, T> {
    ...
    ///
    /// The state lock is released before this method returns while the callback
...
pub trait AppDataHandler {
    /// Package state installed by startup.
    type State: crate::PackageRuntimeState;

    /// Handle one firmware app-data packet and optionally write its response.
    fn handle(
        context: &mut StatefulCallbackContext<'_, Self::State>,
...
#[doc(hidden)]
pub unsafe trait PackageAppDataCallback: AppDataHandler + Sized {
...
#[cfg(any(test, all(not(feature = "test-support"), target_arch = "arm")))]
fn prepare_app_data_reply<'a, T: AppDataHandler>(
    source: crate::PackageStateAccess<'_, T::State>,
    packet: AppDataPacket<'_>,
    buffer: &'a mut AppDataReplyBuffer,
) -> Option<PreparedAppDataReply<'a>> {
    buffer.clear();
...
/// call.
#[cfg(all(not(test), not(feature = "test-support"), target_arch = "arm"))]
pub unsafe extern "C" fn app_data_handler<T: PackageAppDataCallback>(data: *mut u8, len: u32) {
    ...
    let _ = source.with_app_data_reply_buffer(|buffer| {
        if let Some(reply) = prepare_app_data_reply::<T>(source, packet, buffer) {
            reply.send_with(|bytes| {
                let _ = crate::bindings::AppDataBindings::send_app_data_bytes(
                    &crate::bindings::RealBindings,
                    bytes,
                );
            });
        }
...
macro_rules! firmware_stateful_app_data_callback {
    ($name:ident, $callback:ty) => {
        #[doc = "Package-local firmware app-data callback."]
        #[cfg(all(not(test), target_arch = "arm"))]
        #[unsafe(no_mangle)]
        #[inline(never)]
        pub unsafe extern "C" fn $name(data: *mut u8, len: u32) {
            unsafe { $crate::__macro_support::app_data_handler::<$callback>(data, len) };
        }
        ...
        unsafe impl $crate::__macro_support::PackageAppDataCallback for $callback {
            ...
            fn state_type() -> Option<core::any::TypeId> {
                Some(core::any::TypeId::of::<
                    <$callback as $crate::AppDataHandler>::State,
                >())
            }
            ...
            fn state_source() -> $crate::PackageStateAccess<'static, Self::State> {
                unsafe {
                    $crate::__macro_support::__package_state_access(
                        <Self::State as $crate::PackageRuntimeState>::runtime_store(),
                        || {
                            $crate::__macro_support::__firmware_package_state_ptr::<Self::State>(
...
mod tests {
    ...
    struct AppDataDispatchState {
        handled: bool,
        effect_reentered: bool,
        committed_after_effect: bool,
    }

    static APP_DATA_DISPATCH_STATE: crate::PackageStateStore<AppDataDispatchState> =
        crate::PackageStateStore::new();

    impl crate::PackageRuntimeState for AppDataDispatchState {
        fn runtime_store() -> &'static crate::PackageStateStore<Self> {
            &APP_DATA_DISPATCH_STATE
        }
    }

    struct AppDataDispatchHandler;

    impl AppDataHandler for AppDataDispatchHandler {
        type State = AppDataDispatchState;
    ...
    #[test]
    fn app_data_reply_owns_callback_local_bytes() {
        let mut buffer = AppDataReplyBuffer::new();
        let mut callback_local = [101, 31, 4, 2];

        {
            let mut reply = AppDataReply::new(&mut buffer);
            assert_eq!(reply.write(&callback_local), Ok(()));
        }
        callback_local.fill(0);

        assert_eq!(buffer.as_bytes(), Some(&[101, 31, 4, 2][..]));
    }

    #[test]
    fn app_data_reply_enforces_the_vesc_payload_limit() {
        let mut buffer = AppDataReplyBuffer::new();
        let mut reply = AppDataReply::new(&mut buffer);

        assert_eq!(reply.write(&[0; 511]), Ok(()));
        assert_eq!(
            reply.write(&[0; 512]),
            Err(AppDataReplyError::PayloadTooLarge)
        );
    }

    #[test]
    fn prepared_app_data_reply_sends_after_package_state_is_released() {
        let mut state = AppDataDispatchState {
            handled: false,
            effect_reentered: false,
            committed_after_effect: false,
        };
        assert!(unsafe { APP_DATA_DISPATCH_STATE.install(&mut state) });
        let source = crate::PackageStateAccess::runtime(&APP_DATA_DISPATCH_STATE);
        let packet = AppDataPacket::from_bytes(&[101, 31]);
        let mut buffer = AppDataReplyBuffer::new();
        let bindings = crate::test_support::FakeAppDataBindings::new();

        let reply = prepare_app_data_reply::<AppDataDispatchHandler>(source, packet, &mut buffer)
            .expect("prepared reply");
        reply.send_with(|bytes| {
            assert_eq!(bytes, &[101, 31]);
            assert_eq!(
                APP_DATA_DISPATCH_STATE.with(|state| {
                    (
                        state.handled,
                        state.effect_reentered,
                        state.committed_after_effect,
                    )
                }),
                Some((true, true, true))
            );
            assert!(crate::bindings::AppDataBindings::send_app_data_bytes(
                &bindings, bytes,
            ));
        });
        ...
        APP_DATA_DISPATCH_STATE.clear();
    }

    #[test]
    fn callback_without_a_reply_clears_the_reused_runtime_buffer() {
        let mut state = AppDataDispatchState {
            handled: false,
            effect_reentered: false,
            committed_after_effect: false,
        ...
        let source = crate::PackageStateAccess::runtime(&APP_DATA_DISPATCH_STATE);
        let mut buffer = AppDataReplyBuffer::new();
        AppDataReply::new(&mut buffer)
            .write(&[101, 31])
            .expect("seed stale reply");

        assert!(
            prepare_app_data_reply::<AppDataDispatchHandler>(
                source,
                AppDataPacket::from_bytes(&[]),
                &mut buffer,
            )
            .is_none()
        );
    ...
    fn stateful_custom_config_trampolines_validate_and_update_state() {
        ...
        assert!(state.effect_reentered);
        assert!(state.committed_after_effect);
