...
/// Sequential reader for one generated custom-config region.
#[derive(Debug, Clone, Copy)]
pub struct CustomConfigCursor<'a> {
    bytes: &'a [u8],
    offset: usize,
}

impl<'a> CustomConfigCursor<'a> {
    /// Start reading `bytes` at a generated byte offset.
    #[must_use]
    ...
    /// Read one generated boolean byte.
    ...
    /// Read one big-endian `u16` divided by `scale` as a bounded ratio.
    pub fn scaled_ratio(&mut self, scale: f32) -> Option<crate::Ratio> {
        crate::Ratio::from_ratio(f32::from(self.be_u16()?) / scale).ok()
    }

    /// Read one byte through its generated enum conversion.
    pub fn enum_value<T: TryFrom<u8>>(&mut self) -> Option<T> {
...
mod tests {
    use super::{
        ...
        CustomConfigAngularVelocityField, CustomConfigCursor, CustomConfigDurationField,
    ...
    fn config_cursor_decodes_sequential_generated_values() {
        ...
        impl TryFrom<u8> for Mode {
            ...
            fn try_from(value: u8) -> Result<Self, Self::Error> {
                (value == 7).then_some(Self::Active).ok_or(value)
            }
        }

        let mut cursor = CustomConfigCursor::new(&[99, 2, 0x01, 0xf4, 0x13, 0x88, 7], 1);

        assert_eq!(cursor.boolean(), Some(true));
        assert_eq!(cursor.be_u16(), Some(500));
        assert_eq!(
            cursor.scaled_ratio(10_000.0),
            Some(crate::Ratio::from_ratio_const(0.5))
        );
        assert_eq!(cursor.enum_value(), Some(Mode::Active));
        assert_eq!(cursor.offset(), 7);
        assert_eq!(cursor.byte(), None);
    }
