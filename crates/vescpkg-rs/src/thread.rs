...
/// Exclusive access to the optional firmware-reserved Refloat recorder buffer.
///
/// The buffer is supplied by Refloat's data-recording firmware trailer rather
/// than allocated by a package in its control loop.
...
#[cfg(all(not(test), target_arch = "arm"))]
impl PartialEq for FirmwareDataRecorderBuffer {
    fn eq(&self, other: &Self) -> bool {
        self.address == other.address && self.len == other.len
    }
}

#[cfg(all(not(test), target_arch = "arm"))]
impl Eq for FirmwareDataRecorderBuffer {}

#[cfg(all(not(test), target_arch = "arm"))]
// SAFETY: ownership is acquired through the unsafe, one-owner firmware API;
// moving the handle between package callback/thread contexts does not change
// the validity of the firmware-reserved RAM region.
unsafe impl Send for FirmwareDataRecorderBuffer {}

#[cfg(all(not(test), target_arch = "arm"))]
impl FirmwareDataRecorderBuffer {
    /// Discover the optional Refloat firmware buffer from the VESC trailer.
    ///
    /// # Safety
    ///
    /// The VESC interface must be live, and the caller must create at most one
    /// owner for the returned region during the package lifecycle.
    pub(crate) unsafe fn discover() -> Option<Self> {
        const WORD_BYTES: usize = core::mem::size_of::<u32>();
        let info_address = vescpkg_rs_sys::VescIfAbi::BASE_ADDR
            .as_usize()
            .checked_add(vescpkg_rs_sys::VescIfAbi::DATA_RECORDER_INFO_OFFSET)?;
        let info = info_address as *const u32;
        // SAFETY: the caller guarantees a live VESC interface. The optional
        // trailer occupies three aligned 32-bit words at the pinned offset.
        let magic = unsafe { info.read_volatile() };
        if magic != vescpkg_rs_sys::VescIfAbi::DATA_RECORDER_MAGIC {
            return None;
        }
        // SAFETY: the validated trailer layout stores address then byte length.
        let address = unsafe { info.add(1).read_volatile() } as usize;
        // SAFETY: same validated three-word trailer as above.
        let len = unsafe { info.add(2).read_volatile() } as usize;
        let address = NonNull::new(address as *mut u8)?;
        (len >= WORD_BYTES).then_some(Self { address, len })
    }
    ...
    /// Return whether the reserved buffer is empty.
    pub const fn is_empty(&self) -> bool {
        self.len == 0
    }

    /// Copy bytes into the reserved buffer when the range is in bounds.
    pub fn write(&mut self, offset: usize, bytes: &[u8]) -> bool {
        ...
        // SAFETY: acquisition guarantees exclusive ownership of a writable
        // region of `self.len` bytes, and the checked range stays in bounds.
        unsafe {
    ...
    /// Copy bytes from the reserved buffer when the range is in bounds.
    pub fn read(&self, offset: usize, bytes: &mut [u8]) -> bool {
        ...
        // SAFETY: acquisition guarantees a readable region of `self.len`
        // bytes, and the checked range stays in bounds.
        unsafe {
            core::ptr::copy_nonoverlapping(
                self.address.as_ptr().add(offset),
                bytes.as_mut_ptr(),
