...
| GPIO inventory and leases | `gpio_lease` covers all 13 pinned digital enum values plus exclusive acquire/configure/read/release behavior; `stm32_pad` covers explicit raw pad resolution and mode/set/clear forwarding; Refloat's footpad loop reads typed ADC inputs through `Firmware::gpio()` | Low-level STM32 pad access remains explicitly unsafe and needs hardware electrical proof |
