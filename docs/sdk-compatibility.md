...
| Callback ownership | CAN receive callbacks now use typed IDs and callback-scoped payloads; typed PWM, terminal, packet, command-reply, encoder, and IMU registrations use scoped guards or typed callback state; live registrations reject replacement; typed trampolines fail closed after lease drop and the package stop hook disables all typed gates before state teardown; package-stop regressions cover command, terminal, encoder, CAN, and PWM leases | Raw CAN/PWM callback registration remains an explicitly unsafe escape hatch; retained provider callbacks are not forcibly replaced; packet registration remains borrowing and must stay inside its codec lifetime; hardware ordering proof remains open |
