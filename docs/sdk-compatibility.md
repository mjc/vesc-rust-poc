...
| Usage-shaped package ports | `examples/loopback` covers a typed LispBM extension, app-data transport, and explicit EEPROM read/write through `Firmware::eeprom()`; Refloat covers a typed custom-config callback backed by the same EEPROM capability | Probe/config addresses are package-owned; persistence is opt-in and remains firmware policy |
