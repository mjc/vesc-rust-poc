...
| Usage-shaped package ports | `examples/loopback` covers a typed LispBM extension, app-data transport, explicit EEPROM read/write through `Firmware::eeprom()`, scoped mutex/semaphore operations, and distinct clock-domain reads; Refloat covers a typed custom-config callback backed by the same EEPROM capability | Probe/config addresses are package-owned; persistence is opt-in and remains firmware policy |
