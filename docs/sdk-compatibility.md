...
| Callback ownership | CAN, PWM, terminal, packet, command-reply, encoder, and IMU registrations use scoped guards or typed callback state; command replies reject replacement while live | Firmware slots that retain provider callbacks remain subject to shutdown-order and hardware proof |
