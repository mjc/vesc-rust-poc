...
| Callback ownership | CAN, PWM, terminal, packet, command-reply, encoder, and IMU registrations use scoped guards or typed callback state; live registrations reject replacement and packet reset/drop releases the owned codec | Firmware slots that retain provider callbacks remain subject to shutdown-order and hardware proof |
