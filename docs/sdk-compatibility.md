...
| IMU and package-owned AHRS | `imu` covers firmware readiness, typed yaw round trips, vectors, derotation, and callback copies; `ahrs` covers normalized Mahony and Madgwick state | Firmware estimator snapshots remain separate from package-owned algorithms; calibration hardware proof is still open |
