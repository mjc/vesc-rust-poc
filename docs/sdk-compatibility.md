...
| IMU and AHRS | `imu` covers readiness, atomic roll/pitch/yaw, nine-field calibration snapshots, and owned `ATTITUDE_INFO` lifecycle/update snapshots; `ahrs` covers package-owned initial orientation plus normalized Mahony and Madgwick state | Firmware estimator state remains separate from package-owned algorithms; calibration and hardware estimator proof are still open |
