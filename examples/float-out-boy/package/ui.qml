...
function toFixed1Zero(num){if(num!=num){return"-.-";}
return num.toFixed(1).replace("-0.0","0.0");}
function toFixed2Zero(num){if(num!=num){return"-.--";}
return num.toFixed(2).replace("-0.00","0.00");}
function clamp(val,min,max){if(val>max){return max;}
...
function parseVersion(ver){var dotSplit=ver.split(".");if(dotSplit.length<3){return[0,0,0,""];}
var lastPart=dotSplit[2];var dashPos=lastPart.search("-");var patch=lastPart;var suffix="";if(dashPos>0){patch=lastPart.substring(0,dashPos);suffix=lastPart.substring(dashPos+1);}
return[Number(dotSplit[0]),Number(dotSplit[1]),Number(patch),suffix];}
function versionMajorMinor(ver){var arr=ver.split(".");if(arr.length<2){return[0,0];}
return[Number(arr[0]),Number(arr[1])];}
function cmpVersion(a,b){for(var i=0;i<3;i++){if(a[i]<b[i]){return-1;}else if(a[i]>b[i]){return 1;}}
return 0;}
function getDarkThemeColor(name){switch(name){case"normalBackground":return"#303030";break;case"darkBackground":return"#272727";break;case"normalText":return"#b4b4b4";break;case"lightText":return"#dcdcdc";break;case"disabledText":return"#7f7f7f";break;}}
Timer{id:packageConnectionWatchdog
interval:5000
running:true
onTriggered:{state.pkgState=state.s_Disconnected;}}
Timer{running:true
repeat:true
interval:100
...
return name;}
function setName(name){VescIf.storeBleName(VescIf.getLastBleAddr(),name);VescIf.storeSettings();}
function getFwVersion(){var params=VescIf.getLastFwRxParams();return[params.major,params.minor];}}
Connections{id:vescConfig
target:VescIf
...
property real imperialFactor:useImperial?0.621371192:1.0;function onUseImperialUnitsChanged(useImperialUnits){useImperial=useImperialUnits;}}
Connections{id:motorConfig
...
function onParamChangedDouble(src,name,newParam){if(name==="l_temp_motor_start"){tempMotorStart=newParam;}else if(name==="l_temp_fet_start"){tempFetStart=newParam;}else if(name==="l_current_min"){currentMin=newParam;}else if(name==="l_current_max"){currentMax=newParam;}else if(name==="l_in_current_min"){inCurrentMin=newParam;}else if(name==="l_in_current_max"){inCurrentMax=newParam;}}
function onParamChangedInt(src,name,newParam){if(name==="si_battery_cells"){batteryCells=newParam;}}}
Connections{id:packageConfig
target:VescIf
property ConfigParams pkgConfig:VescIf.customConfig(0);property bool updateDue:false
signal update
property Connections pkgConns:Connections{target:packageConfig.pkgConfig
function onParamChangedDouble(src,name,newParam){packageConfig.pendingUpdate();}
function onParamChangedInt(src,name,newParam){packageConfig.pendingUpdate();}
function onParamChangedEnum(src,name,newParam){packageConfig.pendingUpdate();}
function onParamChangedQString(src,name,newParam){packageConfig.pendingUpdate();}
function onParamChangedBool(src,name,newParam){packageConfig.pendingUpdate();}}
property Timer updateTimer:Timer{id:updateTimer
interval:100
onTriggered:{packageConfig.fireDueUpdate();}}
property Timer dueTimer:Timer{id:dueTimer
interval:2000
onTriggered:{packageConfig.fireDueUpdate();}}
function pendingUpdate(){updateTimer.restart();}
function armDueUpdate(){updateDue=true;dueTimer.restart();}
function disarmDueUpdate(){updateDue=false;dueTimer.stop();}
function fireDueUpdate(){if(updateDue){updateDue=false;update();}}
function onCustomConfigLoadDone(){pkgConfig=VescIf.customConfig(0);if(isDefault()){armDueUpdate();}else{update();}}
function configWritten(){disarmDueUpdate();unsetDefault();update();}
function checkConfig(){state.configLoadingError=!pkgConfig;return!!pkgConfig;}
function writeConfig(){if(!checkConfig())return;unsetDefault();vescCommands.customConfigSet(0,pkgConfig);update();}
function isDefault(){if(!checkConfig())return null;return pkgConfig.getParamBool("meta.is_default");}
function unsetDefault(){if(!checkConfig())return;pkgConfig.updateParamBool("meta.is_default",false);}
function setDisabled(disabled){if(!checkConfig())return;pkgConfig.updateParamBool("disabled",disabled);writeConfig();}
function getParamValue(name){if(pkgConfig.isParamDouble(name)){return pkgConfig.getParamDouble(name);}else if(pkgConfig.isParamInt(name)){return pkgConfig.getParamInt(name);}else if(pkgConfig.isParamEnum(name)){return pkgConfig.getParamEnum(name);}else if(pkgConfig.isParamBool(name)){return pkgConfig.getParamBool(name);}else if(pkgConfig.isParamQString(name)){return pkgConfig.getParamQString(name);}else if(pkgConfig.isParamBitfield(name)){return pkgConfig.getParamInt(name);}
return null;}
function getParamType(name){if(pkgConfig.isParamDouble(name)){return"Double";}else if(pkgConfig.isParamInt(name)){return"Int";}else if(pkgConfig.isParamEnum(name)){return"Enum";}else if(pkgConfig.isParamBool(name)){return"Bool";}else if(pkgConfig.isParamQString(name)){return"String";}else if(pkgConfig.isParamBitfield(name)){return"Bitfield";}
return null;}
...
writeConfig();}
function diffConfig(other){var differs=[];var otherMissing=[];var otherSettings={};if(other&&other.settings){otherSettings=Object.assign({},other.settings);if(VescIf.customConfigsLoaded()){for(let subGroup of pkgConfig.getParamSubgroups("General")){for(let paramName of pkgConfig.getParamsFromSubgroup("General",subGroup)){var value=getParamValue(paramName);if(value===null){continue;}
if(otherSettings.hasOwnProperty(paramName)){var otherValue=otherSettings[paramName];if(value!==otherValue){differs.push({"name":paramName,"value":value,"otherValue":otherValue});}
delete otherSettings[paramName];}else{otherMissing.push(paramName);}}}}}
return{"differs":differs,"missing":Object.values(otherSettings),"otherMissing":otherMissing,}}
function setInputTiltRemoteType(type){if(!checkConfig())return;if(pkgConfig.getParamEnum("inputtilt_remote_type",0)!=1){pkgConfig.updateParamEnum("inputtilt_remote_type",1);writeConfig();}}}
...
function getString(dataView,ind){var str="";var size=dataView.getUint8(ind++);for(var j=0;j<size;j++){str+=String.fromCharCode(dataView.getUint8(ind++));}
return[str,ind];}
function getStrings(dataView,ind){var num=dataView.getUint8(ind++);var strs=[];for(var i=0;i<num;i++){var str;[str,ind]=getString(dataView,ind);strs.push(str);}
return[strs,ind];}
...
Settings{id:persistentData
category:"persistentData"
property var lastPackageUpdateCheck
property var packageUpdateSnooze}
QtObject{id:updateChecker
property CodeLoader packageLoader:CodeLoader{id:packageLoader
function getPackageVersion(pkgName){var pkgs=reloadPackageArchive();for(var pkg of pkgs){if(!pkg.isLibrary&&shouldShowPackage(pkg)&&pkg.name===pkgName){var li=pkg.description.match(/<li>Version: [a-zA-Z0-9._-]+<\/li>/g);if(li){var version=li[0];return version.substring(13,version.length-5);}
var md_bullet=pkg.description.match(/\n- Version: [a-zA-Z0-9._-]+\n/g);if(md_bullet){var version=md_bullet[0];return version.substring(11,version.length);}}}
return null;}
...
StatusText{id:statusTextSubject
x:parent.targetX
y:parent.targetY
...
globState:gState
UIHintFrame{}}
UIHintText{anchors.horizontalCenter:statusTextSubject.horizontalCenter
...
var res=Object.assign({},tune);res.settings=[];for(let[name,value]of Object.entries(tune.settings)){res.settings.push({"name":name,"value":value,"type":packageConfig.getParamType(name)});}
return res;}
function convertFromOldFormat(tune){if(!Array.isArray(tune.settings)){return tune;}
var res=Object.assign({},tune);res.settings={};for(let item of tune.settings){res.settings[item.name]=item.value;}
return res;}
function tuneId(slot){return"tune_slot_"+slot;}
function createTune(name,description,settings){var tune={"version":formatVersion,"name":name,"date":new Date(),"package":{"name":packageName,"version":packageVersion},"board":{"id":board.getId(),},"settings":settings};var boardName=board.getName();if(boardName){tune.board.name=boardName;}
if(description){tune.description=description;}
return tune;}
function updateTune(tune,name,description,settings){if(!packageConfig.versionCompatible(tune)){return false;}
tune.name=name;if(description){tune.description=description;}
if(settings){tune.settings=settings;tune.date=new Date();}
tune.version=formatVersion;return true;}
function migrateTune(tune){var[major,minor]=versionMajorMinor(tune.package.version);if(!major){return tune;}
var[pMajor,pMinor]=versionMajorMinor(packageVersion);while(major<pMajor||(major==pMajor&&minor<pMinor)){var versionKey=[major,minor].join(".");if(migrations.hasOwnProperty(versionKey)){[tune,[major,minor]]=migrations[versionKey](tune);}else{minor++;}}
return tune;}
...
fullBackup=backup;}
function globalRestore(){var backup=loadBackup("fullBackup");if(!backup){return;}
applyConfigDialog.show(backup,false,false);}
function idBackup(){var backup=createBackup();if(!backup){return;}
saveBackup("backup_"+board.getId(),backup);idBackupFinished(backup);}
function idRestore(){var backup=loadBackup("backup_"+board.getId());if(!backup){return;}
applyConfigDialog.show(backup,false,false);}
function autoBackup(){var backup=createBackup();if(!backup){return;}
saveBackup("auto_backup_"+board.getId(),backup);autoBackupFinished(backup);}
...
return result;}
Component.onCompleted:{var idBackup=loadBackup("backup_"+board.getId());if(idBackup){idBackupFinished(idBackup);}
var autoBackup=loadBackup("auto_backup_"+board.getId());if(autoBackup){autoBackupFinished(autoBackup);}}}
property var state:QtObject{id:state
...
onCommsStateChanged:{if(commsState==comms_Initialized){commsInitialized();}}
...
function setCapabilities(caps){capLights=caps&cap_Lights;capLightsExternal=caps&cap_LightsExternal;capDataRecord=caps>=(cap_DataRecord>>>0);}
...
readonly property var pkgStateToString:new Map([[s_Disabled,"DISABLED"],[s_Startup,"STARTUP"],[s_Ready,"READY"],[s_Running,"RUNNING"],[s_Connecting,"CONNECTING..."],[s_Disconnected,"DISCONNECTED"],])
...
property string pkgModeString:{if(pkgState!=s_Disconnected){if(pkgMode==m_Flywheel){return"FLYWHEEL";}else if(pkgMode==m_Handtest){return"HANDTEST";}}
return"";}
...
onFpStateChanged:{leftSensor.on=false;rightSensor.on=false;if(fpState&fs_Left){leftSensor.on=true;}
if(fpState&fs_Right){rightSensor.on=true;}}
...
readonly property var setpointAdjustmentTypeToString:new Map([[sat_None,""],[sat_Centering,"centering"],[sat_ReverseStop,"reverse stop"],[sat_Speed,"pushback: speed"],[sat_Duty,"pushback: duty"],[sat_PushbackError,"pushback: error"],[sat_HighVoltage,"pushback: high voltage"],[sat_LowVoltage,"pushback: low voltage"],[sat_Temperature,"pushback: temperature"],])
...
onSetpointAdjustmentTypeChanged:{setpointAdjustmentClass=setpointAdjustmentType>=sat_HighVoltage?sac_Error:setpointAdjustmentType>=sat_Speed?sac_Warning:sac_Normal;setpointAdjustmentTypeString=setpointAdjustmentTypeToString.get(setpointAdjustmentType)??"unknown (%1)".arg(setpointAdjustmentType);}
...
readonly property var stopConditionToString:new Map([[sc_None,""],[sc_Pitch,"Pitch"],[sc_Roll,"Roll"],[sc_SensorHalf,"Sensor Half"],[sc_SensorFull,"Sensor Full"],[sc_ReverseStop,"Reverse Stop"],[sc_Quickstop,"Quickstop"],])
...
readonly property var beepReasonToString:new Map([[0,""],[1,"Low Voltage"],[2,"High Voltage"],[3,"Controller Temp"],[4,"Motor Temp"],[5,"Overcurrent"],[6,"Duty Cycle"],[7,"Foot Sensors"],[8,"Low Battery"],[9,"Board Idle"],[10,"Other"],[11,"Speed"],[12,"BMS: Cell Under Temp"],[13,"BMS: Cell Over Temp"],[14,"BMS: Cell Low Voltage"],[15,"BMS: Cell High Voltage"],[16,"BMS: Cell Balance"],[17,"BMS: Connection Error"],[18,"BMS: BMS Temp"],[19,"Firmware Fault"],])
...
function ticksToMsSinceEpoch(t){if(timeReference===undefined){return undefined;}
return timeReference+t*1000/ticksPerSecond;}
function setTimeReference(t){var timeRef=ticksToMsSinceEpoch(t);var now=Date.now();if(timeRef===undefined||timeRef<now-8000){timeReference=now-t*1000/ticksPerSecond;}}
property var rtDataIdleItems:[]
property var rtDataRuntimeItems:[]
property var rtDataAllItems:[]
property var rtData:newRtData();function newRtData(values){let newData=new Proxy({},{get:function(target,name){return target.hasOwnProperty(name)?target[name]:NaN;}});if(!values){return newData;}
var names=rtDataAllItems;for(var i=0;i<names.length;i++){newData[names[i]]=values[i];}
return newData;}}
QtObject{id:alerts
...
property Connections connections:Connections{target:state
function onTimeReferenceChanged(){alerts.lastAlertsLogTicks=0;}}
readonly property int alert_fwFault:1
readonly property var alertData:new Map([[0,{"name":"None","severity":"WARNING",}],[alert_fwFault,{"name":"Fault","severity":"FATAL",}],])
readonly property var severityColors:new Map([["FATAL",Utility.getAppHexColor("red")],["ERROR",Utility.getAppHexColor("orange")],["WARNING",Utility.getAppHexColor("lightText")],])
property Timer alertsListPollTimer:Timer{running:state.commsState===state.comms_Initialized
repeat:true
interval:500
triggeredOnStart:true
onTriggered:{commands.sendAlertsList(alerts.lastAlertsLogTicks?alerts.lastAlertsLogTicks:undefined);}}
function getActiveAlertData(alertId){if(alertId==alert_fwFault){var data=Object.assign({},alertData.get(alertId));data.name=data.name+": "+activeFwFaultName;return data;}else{return alertData.get(alertId);}}
function getAlertLogData(alertId,data){return alertData.get(alertId);}
function updateActiveAlerts(){if(activeAlertsMask&alert_fwFault){activeAlertHighlight=alert_fwFault;activeAlertCount=1;isFatalActive=true;activeAlerts=[alert_fwFault,];}else{activeAlertHighlight=0;activeAlertCount=0;isFatalActive=false;activeAlerts=[];}}
function receiveAlertsList(dv,ind){activeAlertsMask=dv.getUint32(ind);ind+=4;ind+=4;activeFwFaultCode=dv.getUint8(ind++);if(activeFwFaultCode>0){[activeFwFaultName,ind]=commands.getString(dv,ind);}else{activeFwFaultName="";}
updateActiveAlerts();var count=dv.getUint8(ind++);var ids=[];for(var i=0;i<count;i++){var ticks=dv.getUint32(ind);ind+=4;var record={"ticks":ticks,"id":dv.getUint8(ind++),"active":dv.getUint8(ind++)&0x1,"code":dv.getUint8(ind++),};if(record.code>0){[record.text,ind]=commands.getString(dv,ind);}
var realTime=state.ticksToMsSinceEpoch(ticks);if(realTime===undefined){break;}
record.time=realTime;var key="%1_%2_%3".arg(ticks).arg(record.id).arg(record.active);if(alertsLog.has(key)&&Math.abs(realTime-alertsLog.get(key).time)<5000){continue;}
alertsLog.set(key,record);lastAlertsLogTicks=ticks;alertAdded(record);}}
function clearFatalError(){commands.sendAlertsClearFatal();}}
QtObject{id:lights
...
contentItem:Item{Canvas{id:iconCanvas
anchors.centerIn:parent
implicitWidth:iconSize
implicitHeight:iconSize
contextType:"2d"
onPaint:{var context=getContext("2d");context.reset();if(iconPath){context.strokeStyle=button.strokeColor;context.lineCap="round";iconPath.scale=Qt.size(iconSize/50,iconSize/50);context.path=iconPath;context.lineWidth=2;context.stroke();}
if(fillIconPath){context.fillStyle=button.fillColor;fillIconPath.scale=Qt.size(iconSize/50,iconSize/50);context.path=fillIconPath;context.fill();}}}}
background:Rectangle{id:buttonBg
radius:parent.width/2
opacity:0.9
color:button.customBgColor??button.bgColor
states:[State{name:"change"
when:button.pressed&&!button.customBgColor
PropertyChanges{target:buttonBg;color:Utility.getAppHexColor("lightestBackground");}}]
transitions:Transition{ColorAnimation{property:"color";duration:200;easing.type:Easing.InOutCirc;}}}}
component ToolButton:FloatingToolButton{anchors.margins:0
bgColor:Utility.getAppHexColor("normalBackground")}
component ToggleFloatingToolButton:FloatingToolButton{property bool autonomous:true
...
Behavior on value{NumberAnimation{easing.type:Easing.OutCirc
duration:100}}
Behavior on topValue{NumberAnimation{easing.type:Easing.OutCirc
duration:100}}
onValueAngleChanged:canvas.requestPaint();onTopAngleChanged:canvas.requestPaint();Canvas{id:canvas
anchors.fill:parent
contextType:"2d"
Path{id:dialBackground
PathAngleArc{centerX:width*0.5
centerY:centerX
...
onPaint:{var context=getContext("2d");context.reset();context.lineWidth=lineWidth;context.strokeStyle=Utility.getAppHexColor("lightBackground");context.path=dialBackground;context.stroke();if(!allowNegative){context.strokeStyle=Utility.getAppHexColor("lightestBackground");context.path=topFill;context.stroke();}
if(warnThresholdAbs>0&&Math.abs(value)>warnThresholdAbs){context.strokeStyle=Utility.getAppHexColor("orange");}else{if(valueAngle>0){context.strokeStyle=Utility.getAppHexColor("lightAccent");}else{context.strokeStyle=Utility.getAppHexColor("green");}}
context.path=valueFill;context.stroke();}}
Rectangle{id:zeroMarker
width:1
...
component BaseDialog:Dialog{x:10
...
component PlotData:QtObject{property int sampleSize:0
...
property Timer receiveTimeout:Timer{interval:1000
onTriggered:{recPlotData.requestPlot(true);}}
function fetch(ticksPerSec){reset();ticksPerSecond=ticksPerSec;noData=false;timeout=false;requestPlot();}
function requestPlot(isARetry){var reqTries=requestTries;if(!isARetry){reqTries=1;}else if(reqTries>5){timeout=true;return;}
if(!initialized){commands.sendDataRecordRequest(true);}else{commands.sendDataRecordRequest(false,sampleNr);}
reqTries++;requestTries=reqTries;receiveTimeout.start();}
function receiveHeader(dv,ind){if(initialized){return;}
receiveTimeout.stop();sampleCount=dv.getUint32(ind);ind+=4;var sampleSize=dv.getUint8(ind++);var ids=[];for(var i=0;i<sampleSize;i++){var id="";var size=dv.getUint8(ind++);for(var j=0;j<size;j++){id+=String.fromCharCode(dv.getUint8(ind++));}
ids.push(id);}
initSeries(ids,true);if(sampleCount===0){noData=true;}else{requestPlot();}}
...
function init(ticksPerSec){ticksPerSecond=ticksPerSec;reset();plotData=plotData;}
...
trueFillIconPath:Path{PathSvg{path:"M 13 13 L 13 37 L 37 37 L 37 13 L 13 13"}}
falseFillIconPath:Path{PathAngleArc{centerX:25;centerY:25;radiusX:12;radiusY:12;sweepAngle:360;}}
...
onClicked:{commands.sendDataRecordControl(commands.dr_AUTOSTART,checked);}}
...
onPinchStarted:{initialPlotWindow=plotCanvas.window;initialX=plotMouse.x(pinch.center);plotMover.grab();}
onPinchUpdated:{plotMover.move(initialX-plotMouse.x(pinch.center),pinch.scale!=lastScale);plotMover.zoom(initialPlotWindow/pinch.scale,plotMouse.x(pinch.center));lastScale=pinch.scale;}
onPinchFinished:{plotMouse.release();}
MouseArea{id:plotMouse
anchors.fill:parent
...
function x(obj){return obj.x-plotCanvas.padding;}
function dist2d(event){var dx=initialX-x(event);var dy=initialY-event.y;return Math.sqrt(dx*dx+dy*dy);}
...
plotCanvas.fadeCursor(1);return;}
plotCanvas.fadeCursor(clamp(1-(yd-snap)/100,0,1));}}else{if(Math.abs(d)>snap){moving=true;}
plotMover.move(d);}}
onReleased:{release();}
onPressAndHold:{if(cancelAction){return;}
if(!moving&&(dist2d(mouse)<snap||!holdCursor)){holdCursor=true;moveCursor=true;plotCanvas.spawnCursorCoord(x(mouse));}}}
...
Connections{target:plotData
function onDataUpdate(){if(isNaN(plotCanvas.position)){if(plot.followEnd){plotCanvas.position=plotData.timeEnd-plotCanvas.window;plotMover.followPosition();}else{plotCanvas.position=plotData.timeStart;plotCanvas.window=plotData.timeRange;}}
if(plot.followEnd&&plotMover.following){plotMover.positionFollower=plotData.timeEnd-plotCanvas.window;}}
function onScalesUpdate(){plotCanvas.requestPaint();}}
onVisibleChanged:requestPaint();onPositionChanged:requestPaint();onWindowChanged:requestPaint();onCursorChanged:requestPaint();onCursorOpacityChanged:requestPaint();QtObject{id:plotMover
...
onAnimWinChanged:{zoom(animWin,zoomCenter);}
onPositionFollowerChanged:{if(following){plotCanvas.position=positionFollower;}}
Behavior on positionFollower{id:positionFollowerBehavior
NumberAnimation{easing.type:Easing.OutExpo
duration:800}}
function followPosition(){positionFollowerBehavior.enabled=false;positionFollower=plotCanvas.position;positionFollowerBehavior.enabled=true;following=true;}
function grab(){moveReferencePos=plotCanvas.position;animWin=plotCanvas.window;lastDist=0;lastTime=0;speed=0;grabbed=true;moving=false;rolling=false;following=false;zoomAnim.stop();bounceBack.stop();bounceBackZoom.stop();}
function release(){if(grabbed){snapToBounds();}
grabbed=false;}
function blunt(x,a){return a*Math.exp(x/a)-a;}
function moveRelative(pos,x){moveUnbluntedPos=plotCanvas.toPlotPos(pos,x);var newPos=moveUnbluntedPos;var endPos=plotData.timeEnd-plotCanvas.window;var startPos=plot.followEnd&&plotCanvas.window>plotData.timeRange?endPos:plotData.timeStart;var startDist=newPos-startPos;if(startDist<0){newPos+=-startDist+blunt(startDist/plotCanvas.window,bounceOffset)*plotCanvas.window;}
var endDist=endPos-newPos;if(endDist<0){newPos+=endDist-blunt(endDist/plotCanvas.window,bounceOffset)*plotCanvas.window;}
var ret=plotCanvas.toCanvasPos(newPos);plotCanvas.position=newPos;return ret;}
function move(dist,isZooming){moveRelative(moveReferencePos,dist);var now=Date.now();var dt=now-lastTime;if(dt===0){return;}
var alpha=0.9;if(isZooming){zoomDampen=alpha;}else{alpha-=zoomDampen;}
var s=(dist-lastDist)/dt*1000;speed+=alpha*(s-speed);zoomDampen=Math.max(zoomDampen*0.98-0.002,0);moving=true;lastDist=dist;lastTime=now;}
function zoomPosOffset(oldWindow,newWindow,center){var perc=oldWindow/newWindow-1;return(plotCanvas.toPlotPos(plotCanvas.position,center,newWindow)-plotCanvas.position)*perc;}
function zoom(newWindow,center){bounceBackZoom.stop();lastZoomCenter=center;var maxWin=maxWindow;var maxDist=maxWin-newWindow;if(maxDist<0){newWindow=(1-blunt(maxDist/maxWin,bounceOffset*2))*maxWin;}
var minDist=newWindow-minWindow;if(minDist<0){newWindow=(1+blunt(minDist/minWindow,bounceOffset*2))*minWindow;}
var posOffset=zoomPosOffset(plotCanvas.window,newWindow,center);moveReferencePos+=posOffset;plotCanvas.position=plotCanvas.position+posOffset;plotCanvas.window=newWindow;}
function flick(){grabbed=false;rolling=true;}
function snapToBounds(){var windowTo=clamp(plotCanvas.window,minWindow,maxWindow);bounceBackZoom.stop();bounceBackZoom.to=windowTo;bounceBackZoom.start();if(plot.followEnd&&(plotCanvas.window>plotData.timeRange||plotCanvas.position>plotData.timeEnd-plotCanvas.window*1.05)){plotMover.followPosition();}else{bounceBack.stop();var newPos=plotCanvas.position+zoomPosOffset(plotCanvas.window,windowTo,lastZoomCenter);bounceBack.to=clamp(newPos,plotData.timeStart,plotData.timeEnd-windowTo);bounceBack.start();}}
function zoomAnimated(by,center){zoomCenter=center;if(!zoomAnim.running){zoomAnim.to=animWin/by;}else{zoomAnim.stop();zoomAnim.to/=by;}
zoomAnim.start();}}
Timer{id:plotFlickTimer
repeat:true
interval:dt*1000
running:plotCanvas.visible
property real dt:1/30
onTriggered:{var sign=plotMover.speed<0?-1:1;if(plotMover.speed*sign>0){var d=plotMover.speed*dt;if(plotMover.rolling){var realD=plotMover.moveRelative(plotMover.moveUnbluntedPos,d);if(sign*realD<sign*d-0.000001){plotMover.speed=realD/dt;plotMover.speed-=sign*0.1;}else{plotMover.speed*=0.98;plotMover.speed-=sign*2;}}else if(!plotMover.moving){plotMover.speed*=0.7;plotMover.speed-=sign*20;}
plotMover.moving=false;}
if(plotMover.speed*sign<=0){plotMover.speed=0;if(plotMover.rolling){plotMover.snapToBounds();}
plotMover.rolling=false;}}}
NumberAnimation{id:zoomAnim
target:plotMover
property:"animWin"
easing.type:Easing.OutQuad
duration:200
onFinished:{plotMover.release();}}
NumberAnimation{id:bounceBack
target:plotCanvas
property:"position"
...
period:0.5}
duration:300}
NumberAnimation{id:bounceBackZoom
target:plotCanvas
property:"window"
easing{type:Easing.OutBack
amplitude:2.0
period:0.5}
duration:300}
function toPlotPos(oldPos,x,win){if(win===undefined){win=window;}
return oldPos+x/xLength*win;}
function toCanvasPos(pos){return(pos-position)/window*xLength;}
function isCursorPosValid(pos){if(pos<plotData.timeStart||pos<position||pos>position+window||pos>plotData.timeEnd){return false;}
return true;}
function spawnCursorCoord(x){spawnCursor(toPlotPos(position,x));}
function spawnCursor(cursorPos){if(!isCursorPosValid(cursorPos)){return;}
for(var i=0;i<plotData.data.length;i++){var time1=plotData.data[i][0];if(cursorPos<time1){if(i===0){cursor=plotData.data[i];}else{var time2=plotData.data[i-1][0];if(dist(cursorPos,time1)<dist(cursorPos,time2)){cursor=plotData.data[i];}else{cursor=plotData.data[i-1];}}
break;}}}
function getCursorPosIfCan(){if(!cursor){return null;}
var pos=cursor[0];if(!isCursorPosValid(pos)){return null;}
return toCanvasPos(pos);}
function fadeCursor(a=0){a=a*a*a;if(a<=0.05){cursor=null;}else{cursorOpacity=a;}}
function reset(){position=NaN;window=plot.startWindow*plot.ticksPerSecond;cursor=null;}
function pointYCoord(point,n){return point[n+1]*yLength*plotData.scales[n];}
function timeFormat(s,decimals){var res="";if(s>60){var m=Math.floor(s/60);s-=m*60;if(m>60){var h=Math.floor(m/60);m=String(m-h*60).padStart(2,"0");res+=h+":";}
res+=m+":";}
if(s<10){res+="0";}
if(decimals!==undefined){res+=s.toFixed(decimals);}else{res+=s.toFixed(6)/1;}
return res;}
function paintXAxis(ctx,yMax,yMin){ctx.beginPath();ctx.lineWidth=1;ctx.strokeStyle=getDarkThemeColor("disabledText");ctx.moveTo(-padding,0);ctx.lineTo(xLength+padding,0);ctx.stroke();if(window==0||xLength==0){return;}
var basePixelsPerTick=60;var calcTick=function(extraWidth){var mult=1;var tick=window/xLength*(basePixelsPerTick+extraWidth);while(tick<1){mult*=0.1;tick*=10;}
while(tick>10){mult*=10;tick*=0.1;}
if(tick>5){tick=5;}else if(tick>2){tick=2;}else{tick=1;}
return tick*mult;}
var digitSize=ctx.measureText("8").width;var sepSize=ctx.measureText(":").width;var sampleTime=plotData.timeEnd;var decs=Math.max(-Math.log10(window/xLength*basePixelsPerTick),0);var estimatedExtraWidth=decs*digitSize+sepSize;if(sampleTime>=60){estimatedExtraWidth+=digitSize+sepSize;}
if(sampleTime>=600){estimatedExtraWidth+=digitSize;}
if(sampleTime>=3600){estimatedExtraWidth+=digitSize+sepSize;}
var tick=calcTick(Math.max(0,estimatedExtraWidth-10)*3.6);ctx.beginPath();ctx.textAlign="center";ctx.fillStyle=getDarkThemeColor("disabledText");ctx.strokeStyle=getDarkThemeColor("darkBackground");var notch=Math.floor(position/tick)*tick;var maxNotch=Math.ceil((position+window)/tick)*tick;var ticksPerSec=plot.ticksPerSecond;while(notch<=maxNotch){var coord=toCanvasPos(notch);ctx.moveTo(coord,-yMax);ctx.lineTo(coord,-yMin);var decimals=Math.max(Math.ceil(-Math.log10(tick/ticksPerSec)),0);ctx.fillText(timeFormat(notch/ticksPerSec,decimals),coord,-yMin+15);ctx.stroke();notch+=tick;}
ctx.stroke();}
function paintCursor(ctx,yMax,yMin){var time=cursor[0];if(time<position||time>position+window){return;}
var originalAlpha=ctx.globalAlpha;ctx.globalAlpha=cursorOpacity;ctx.beginPath();ctx.lineWidth=1;ctx.strokeStyle=getDarkThemeColor("normalText");var cursorCoord=toCanvasPos(cursor[0]);ctx.moveTo(cursorCoord,-yMax-3);ctx.lineTo(cursorCoord,-yMin+3);ctx.stroke();ctx.textAlign="center";ctx.textBaseline="top";ctx.fillStyle=getDarkThemeColor("normalText");var text=timeFormat(time/plot.ticksPerSecond);var rectWidth=ctx.measureText(text).width+20;var rectHalfWidth=rectWidth/2;var removePadding=padding+5;var xTextCoord=clamp(cursorCoord,rectHalfWidth-removePadding,xLength-rectHalfWidth+removePadding-1);var yTextCoord=-yMin+5;ctx.rect(xTextCoord-rectHalfWidth,yTextCoord,rectWidth,14);var pc=plot.color;ctx.fillStyle=Qt.rgba(pc.r,pc.g,pc.b,0.7);ctx.fill();ctx.fillStyle=getDarkThemeColor("normalText");ctx.fillText(text,xTextCoord,yTextCoord);ctx.textBaseline="alphabetic";var textHeight=10;var yOffset=4;var labelHeight=textHeight+yOffset;var topLimit=yMax-textHeight-1;var bottomLimit=yMin+1;var pd=plotData;var series=pd.series;var textX=cursorCoord;var textFormat;if(cursorCoord>xLength/2){textX-=4;ctx.textAlign="right";textFormat="%2  %1";}else{textX+=4;ctx.textAlign="left";textFormat="%1  %2";}
var vals=[];for(var n=0;n<pd.sampleSize;n++){if(!series[n].visible){continue;}
...
prevTime=t;}}
if(lastFour.length<4&&distanceSum/lastFour.length<dotsThreshold){paintDots=false;}
minDistance=Math.min(minDistance,distanceSum);var dotDistThreshold=Math.max(minDistance/lastFour.length*3,10);paintDotsHyst=paintDots;var step=0.5/Screen.devicePixelRatio;var justOutOfMinMax=false;var halfASecondPixels=0.5*plot.ticksPerSecond*xLen/win;var srsSorted=pd.seriesSorted;for(var k=srsSorted.length-1;k>=0;k--){var series=srsSorted[k];if(!series.visible){continue;}
ctx.beginPath();var color=series.color;ctx.strokeStyle=color;ctx.fillStyle=color;var n=series.dataOrder;var nScale=scales[n];var min=-1e30;var max=1e30;var lastX=-0.001;var lastSampleX=-0.001;var lastY=0;var lastDrawnX=0;var minMaxMode=false;var drawnLastDot=false;for(var i=start;i<end;i++){var point=data[i];var nPoint=point[n+1];var x=(point[0]-pos)/win*xLen;if(nPoint!==nPoint){lastSampleX=x;continue;}
...
ctx.stroke();}}
onPaint:{if(!visible){return;}
var ctx=getContext("2d");ctx.reset();if(!plotData.dataReady){return;}
var yMax=yLength*plotData.positiveRatio;var yMin=yLength*(plotData.positiveRatio-1);ctx.translate(padding,yMax+toolbarHeight);paintXAxis(ctx,yMax,yMin);paintSeries(ctx,yMax,yMin);if(cursor!==null){paintCursor(ctx,yMax,yMin);}}}}}
component OverlayPlot:Plot{anchors.centerIn:parent
...
Glow{property bool red:globState.isError
...
color:{if(red){return Utility.getAppHexColor("red");}else if(dark){return Utility.isDarkMode()?Utility.getAppHexColor("black"):Utility.getAppHexColor("disabledText");}else if(blue){return Utility.getAppHexColor("lightAccent");}else{return Utility.getAppHexColor("lightText");}}
source:statusTextState}
Text{id:statusTextMode
...
Glow{anchors.fill:statusTextMode
...
text:alert.name}
Glow{anchors.fill:statusTextAlert
radius:Math.round(0.12*unit)
samples:radius*2+1
opacity:0.3
...
function locateButton(comp,name){for(var i=0;i<comp.children.length;i++){var child=comp.children[i];if(child.toString().startsWith("Button_QMLTYPE")&&child.text===name){return child;}
var res=locateButton(child,name);if(res){return res;}}
return null;}
Timer{id:onStartupTimer
interval:10
onTriggered:{updateChecker.check();}}
property var customConfigWriteButton:null
Component.onCompleted:{if(typeof confCustomLoader!==typeof undefined){print("Float Out Boy: Hooking custom config Write button...");customConfigWriteButton=locateButton(confCustomLoader.item,"Write");if(customConfigWriteButton){customConfigWriteButton.clicked.connect(packageConfig.configWritten);}else{print("Float Out Boy: Failed to locate Write button.");}}
if(preferences.showWelcomeDialog){welcomeDialog.show();}
onStartupTimer.start();}
Component.onDestruction:{if(typeof confCustomLoader!==typeof undefined){print("Float Out Boy: Unhooking custom config Write button...");if(customConfigWriteButton){customConfigWriteButton.clicked.disconnect(packageConfig.configWritten);}else{print("Float Out Boy: Write button was not set.");}}}
Dl.FileDialog{id:jsonSaveDialog
title:"Choose a file to save"
nameFilters:["JSON (*.json)"]
selectExisting:false
property string jsonData
function show(data){if(!Utility.requestFilePermission()){VescIf.emitMessageDialog("File Permissions","Unable to request file system permission.",false,false);return;}
jsonData=JSON.stringify(data,null,4);open();}
onAccepted:{var fUrl=fileUrl.toString();if(!fUrl.toLowerCase().endsWith(".json")){fUrl+=".json";}
var request=new XMLHttpRequest();request.open("PUT",fUrl,false);request.send(jsonData);if(request.status===0){VescIf.emitStatusMessage("File saved",true);}else{VescIf.emitStatusMessage("Save failed: %1".arg(request.status),false);}}}
Dl.FileDialog{id:jsonLoadDialog
title:"Choose a file to load"
nameFilters:["JSON (*.json)"]
selectExisting:true
function show(){if(!Utility.requestFilePermission()){VescIf.emitMessageDialog("File Permissions","Unable to request file system permission.",false,false);return;}
applyConfigDialog.open();open();}
onAccepted:{var fUrl=fileUrl.toString();var request=new XMLHttpRequest();request.open("GET",fUrl,false);request.send(null);if(request.status===200){applyConfigDialog.set(JSON.parse(request.responseText));}else{VescIf.emitStatusMessage("Opening file failed: %1".arg(request.status),false);}}
onRejected:{applyConfigDialog.close();}}
BaseDialog{id:settingsDialog
implicitHeight:fullHeight
title:"Settings"
standardButtons:Dialog.Close
function show(tab){settingsTabBar.currentIndex=tab||0;open();}
component RightSwitch:RowLayout{Layout.fillWidth:true
property string label
property alias checked:swtch.checked
LText{Layout.fillWidth:true
text:parent.label}
Switch{id:swtch}}
component IntTextField:RowLayout{Layout.fillWidth:true
property string label
property string valueUnit:""
property int value
...
onTextChanged:{if(text!==""){var number=Number(text);if(number>maxValue){text=prevText;}else if(number<minValue){parent.value=Math.max(number,minValue);}else{parent.value=number;prevText=text;}}}
onActiveFocusChanged:{if(!activeFocus){text=parent.value;}}
Component.onCompleted:{text=parent.sourceValue;prevText=text;}}
LText{text:parent.valueUnit}}
component Heading:LText{Layout.fillWidth:true
Layout.topMargin:10
horizontalAlignment:TextInput.AlignHCenter
font.pointSize:16}
component SubHeading:LText{Layout.fillWidth:true
Layout.topMargin:10
horizontalAlignment:TextInput.AlignHCenter
font.pointSize:14}
component OptionDescription:NText{Layout.fillWidth:true
...
horizontalAlignment:TextInput.AlignHCenter
wrapMode:Text.WordWrap}
component SetupDescription:NText{Layout.fillWidth:true
horizontalAlignment:TextInput.AlignHCenter
wrapMode:Text.WordWrap}
ColumnLayout{width:settingsDialog.availableWidth
height:parent.height
TabBar{id:settingsTabBar
Layout.fillWidth:true
clip:true
Repeater{model:["Setup","Preferences"]
TabButton{text:modelData}}
background:Rectangle{opacity:1
color:Utility.getAppHexColor("lightBackground");}
onCurrentIndexChanged:{settingsStackLayout.currentIndex=currentIndex;}}
StackLayout{id:settingsStackLayout
Layout.fillWidth:true
...
Heading{text:"Disable Package"}
SetupDescription{text:"Before running the motor wizard, you need to disable the package to prevent the two fighting over the motor. After configuring your motor, you can re-enable the package."}
Button{Layout.alignment:Qt.AlignHCenter
text:state.pkgState===state.s_Disabled?"Enable Float Out Boy":"Disable Float Out Boy"
onClicked:{packageConfig.setDisabled(state.pkgState!==state.s_Disabled);}}
Heading{text:"Handtest"}
SetupDescription{text:"After finishing setup, you can turn on the Handtest mode and engage your board by hand to verify that it balances in the correct direction. Motor currents will be limited to a safe level."}
Button{Layout.alignment:Qt.AlignHCenter
...
onClicked:{commands.sendHandtest(state.pkgMode===state.m_Normal);}}
SetupDescription{visible:state.pkgMode===state.m_Handtest
...
Heading{text:"Backup / Restore"}
SetupDescription{text:"Backups are stored in VESC Tool's internal storage."}
SubHeading{text:"Automatic Board Backup"}
SetupDescription{id:setupAutoBackupDate
font.pointSize:10
horizontalAlignment:TextInput.AlignHCenter
property var backup:null
visible:!!backup
text:backup?Qt.formatDateTime(backup.date):""
Connections{target:tuneManager
function onAutoBackupFinished(backup){setupAutoBackupDate.backup=backup;}}}
Button{Layout.alignment:Qt.AlignHCenter
text:"View"
onClicked:{tuneManager.autoRestore();}}
SubHeading{text:"Board Backup"}
SetupDescription{id:setupIdBackupDate
font.pointSize:10
horizontalAlignment:TextInput.AlignHCenter
property var backup:null
visible:!!backup
text:backup?Qt.formatDateTime(backup.date):""
Connections{target:tuneManager
function onIdBackupFinished(backup){setupIdBackupDate.backup=backup;}}}
RowLayout{Layout.fillWidth:true
Layout.alignment:Qt.AlignHCenter
Button{Layout.alignment:Qt.AlignHCenter
text:"Backup"
onClicked:{tuneManager.idBackup();}}
Button{Layout.alignment:Qt.AlignHCenter
text:"View"
onClicked:{tuneManager.idRestore();}}}
SubHeading{text:"Shared Backup"}
SetupDescription{font.pointSize:10
horizontalAlignment:TextInput.AlignHCenter
...
Button{Layout.alignment:Qt.AlignHCenter
text:"Backup"
onClicked:{tuneManager.globalBackup();}}
Button{Layout.alignment:Qt.AlignHCenter
text:"View"
onClicked:{tuneManager.globalRestore();}}}}}
ScrollView{Layout.fillWidth:true
Layout.fillHeight:true
contentWidth:availableWidth
clip:true
ColumnLayout{anchors.fill:parent
IntTextField{label:"Speed dial max"
valueUnit:vescConfig.speedUnit
sourceValue:preferences.speedDialMax
minValue:10
maxValue:200
onValueChanged:{preferences.speedDialMax=value}}
RightSwitch{id:prefShowBattVoltage
checked:true
label:"Show battery voltage"}
RightSwitch{id:prefShowBattVoltagePerCell
label:"Show battery voltage per cell"}
RightSwitch{id:prefBattCurrentLog
checked:true
label:"Use logarithmic scale for battery current bar"}
IntTextField{label:"Temperature warning offset"
valueUnit:"°C"
sourceValue:preferences.tempWarningOffset
onValueChanged:{preferences.tempWarningOffset=value}}
OptionDescription{text:"Motor/Controller temperatures will be highlighted red if above TEMP - OFFSET, where TEMP is your Motor/MOSFET Temp Cutoff Start in Motor config. The package beeps when temperatures cross the TEMP - 3°C threshold and pushback triggers at TEMP - 2°C."}
RowLayout{Layout.fillWidth:true
Layout.rightMargin:20
LText{text:"Number of tune slots"}
Slider{id:prefTuneSlotCount
Layout.fillWidth:true
from:2
...
font.pointSize:16}}
RightSwitch{id:prefShowTuneDiffCount
checked:false
label:"Show changed options count on tunes"}
IntTextField{label:"Realtime plot maximum window"
valueUnit:"minutes"
sourceValue:preferences.plotMaxWindowMinutes
minValue:1
maxValue:1000
onValueChanged:{preferences.plotMaxWindowMinutes=value}}
RightSwitch{id:prefShowWelcomeDialog
checked:true
label:"Show Welcome dialog"}
RightSwitch{id:prefCheckForNewVersions
checked:true
label:"Check for new package versions"}
Button{Layout.alignment:Qt.AlignHCenter
text:"Show Tips && Tricks"
onClicked:{uiHints.reset();uiHintsRect.fill(true);settingsDialog.close();}}}}}}}
BaseDialog{id:newPackageVersionDialog
parent:ApplicationWindow.overlay
title:"New Package Version"
standardButtons:Dialog.Ok|Dialog.Abort
property string version
function show(newVersion){version=newVersion;standardButton(Dialog.Abort).text="Snooze for one month"
open();}
onRejected:{persistentData.packageUpdateSnooze=Date.now();}
NText{width:newPackageVersionDialog.availableWidth
font.pointSize:12
horizontalAlignment:Text.AlignHCenter
wrapMode:TextEdit.WordWrap
text:"New Float Out Boy package version available: %1\n\nIt's recommended you update it in the Package Store.\n\nYour package configuration will be migrated automatically, no extra steps needed unless mentioned in package description. Having a manual XML or Start Page \"Backup Configs\" backup is still recommended.".arg(newPackageVersionDialog.version)}}
BaseDialog{id:editBoardNameDialog
title:"Set Board Name"
standardButtons:Dialog.Save|Dialog.Cancel
onAccepted:{board.setName(boardNameInput.text);boardNameText.load();}
TextField{id:boardNameInput
width:editBoardNameDialog.availableWidth
maximumLength:30
text:boardNameText.name}}
BaseDialog{id:editTuneDialog
...
function show(slot){tuneSlot=slot;tune=tuneManager.loadTune(slot);editTuneDiff.setModel(packageConfig.diffConfig(tune).differs);editTuneDiff.reset();standardButton(Dialog.Save).enabled=!!tune;if(!!tune){standardButton(Dialog.Discard).text="Erase"}
open();}
function tuneAttr(key){return tune?tune[key]:"";}
onAccepted:{if(saveTuneNameInput.text){if(!tune){tune=tuneManager.createTune(saveTuneNameInput.text,saveTuneDescInput.text,packageConfig.fetchConfig(true));}else{var success=tuneManager.updateTune(tune,saveTuneNameInput.text,saveTuneDescInput.text,saveTuneOverwrite.checked?packageConfig.fetchConfig(true):null);if(!success){return;}}
tuneManager.saveTune(tuneSlot,tune);VescIf.emitStatusMessage(saveTuneNameInput.text+" saved",true);}}
onDiscarded:{tuneManager.eraseTune(tuneSlot);VescIf.emitStatusMessage(tune.name+" erased",true);close();}
ColumnLayout{width:editTuneDialog.availableWidth
...
RowLayout{Layout.fillWidth:true
LText{text:"Name:"}
TextField{id:saveTuneNameInput
Layout.fillWidth:true
maximumLength:30
text:editTuneDialog.tuneAttr("name")
onTextEdited:{editTuneDialog.standardButton(Dialog.Save).enabled=!!text;}}}
LText{Layout.fillWidth:true
text:"Description:"}
TextArea{id:saveTuneDescInput
Layout.fillWidth:true
implicitHeight:80
wrapMode:TextEdit.WordWrap
text:editTuneDialog.tuneAttr("description")||""}
ConfigDiff{id:editTuneDiff
Layout.fillWidth:true
dialog:editTuneDialog
isTune:true}
CheckBox{id:saveTuneOverwrite
checked:true
visible:editTuneDialog.tune&&!editTuneDiff.isEmpty
text:"Overwrite with current tune"}}}
BaseDialog{id:tuneArchiveDialog
implicitHeight:fullHeight
clip:true
title:"Tune Archive"
...
spacing:15
clip:true
LText{id:tuneArchiveDownloadStatus
Layout.fillWidth:true
Layout.alignment:Qt.AlignHCenter
font.pointSize:11
horizontalAlignment:Text.AlignHCenter
wrapMode:Text.WordWrap}
ListView{id:tuneArchiveTunesList
Layout.fillWidth:true
Layout.fillHeight:true
Layout.alignment:Qt.AlignHCenter
spacing:10
...
BaseDialog{id:alertsDialog
title:"Alerts"
standardButtons:Dialog.Close
function show(){alertsDialogListModel.set(alerts.alertsLog);activeAlertsItem.visible=alerts.activeAlertCount>0;open();}
Connections{target:alerts
function onAlertAdded(alert){if(alertsDialog.visible){alertsDialogListModel.add(alert);}}
function onActiveAlertCountChanged(){if(alerts.activeAlertCount>0){activeAlertsItem.visible=true;}}}
ScrollView{anchors.fill:parent
contentWidth:alertsDialog.availableWidth
clip:true
ColumnLayout{anchors.fill:parent
component ErrHeader:NText{Layout.fillWidth:true
font.pixelSize:16
horizontalAlignment:Text.AlignHCenter}
component ErrText:NText{Layout.fillWidth:true
font.pixelSize:12
font.family:"Mono"
color:Utility.getAppHexColor("red")}
component ErrDesc:NText{Layout.fillWidth:true
font.pixelSize:12
horizontalAlignment:Text.AlignHCenter
wrapMode:Text.WordWrap}
ErrHeader{text:"Package Disabled"
visible:state.pkgState===state.s_Disabled}
ErrDesc{text:"Float Out Boy package is disabled. After you're done with board setup, re-enable it."
visible:state.pkgState===state.s_Disabled}
Button{Layout.alignment:Qt.AlignHCenter
text:"Enable Float Out Boy"
visible:state.pkgState===state.s_Disabled
onClicked:{packageConfig.setDisabled(false);}}
ErrHeader{text:"Connection Errors"
visible:state.configLoadingError||state.pkgState===state.s_Disconnected}
ErrText{text:"Float Out Boy Cfg failed to load"
visible:state.configLoadingError}
ErrDesc{text:"Reconnecting to the board usually fixes this. If problem persists, try reinstalling the package."
visible:state.configLoadingError}
ErrText{text:"Package disconnected"
visible:state.pkgState===state.s_Disconnected}
ErrDesc{text:"Package is not responding. May occur temporarily with spotty connection. If board doesn't engage, the package is dead. Restart the board, if problem persists, try reinstalling the package.";visible:state.pkgState===state.s_Disconnected}
Item{id:activeAlertsItem
Layout.fillWidth:true
Column{id:activeAlertsColumn
width:parent.width
property bool resetHeight:true
ErrHeader{width:parent.width
text:"Active Alerts"}
Item{width:parent.width
height:5}
Repeater{model:alerts.activeAlerts
ErrText{property var alert:alerts.getActiveAlertData(modelData)
text:alert.name
color:alerts.severityColors.get(alert.severity)}}
onHeightChanged:{if(resetHeight||!activeAlertsItem.visible||activeAlertsItem.implicitHeight<height){activeAlertsItem.implicitHeight=height;resetHeight=false;}}}
onVisibleChanged:{activeAlertsColumn.resetHeight=true;}}
ErrHeader{text:"Fatal Error"
visible:state.fatalError}
ErrDesc{text:"VESC reported a fault. Be very careful. You should not continue riding before you fix the root cause. "+(alerts.isFatalActive?"The Fault is still active, you need to resolve the fault and then clear the error.":"The package will stay in an error state until you clear the error.")
visible:state.fatalError}
Button{Layout.alignment:Qt.AlignHCenter
text:"Clear Fatal Error"
visible:state.fatalError
enabled:!alerts.isFatalActive
onClicked:{alerts.clearFatalError();}}
ErrHeader{text:"Alert Log"}
Rectangle{color:Utility.getAppHexColor("darkBackground")
Layout.fillWidth:true
Layout.minimumHeight:200
ListView{id:alertsDialogList
anchors.fill:parent
anchors.margins:6
clip:true
model:ListModel{id:alertsDialogListModel
function add(alert){var wasAtEnd=alertsDialogList.atYEnd;var model=Object.assign({},alert);Object.assign(model,alerts.getAlertLogData(alert.id));if(!alert.active){model.name=model.name+" ended";}else{if(alert.id===alerts.alert_fwFault){model.name=model.name+": %1 (%2)".arg(model.text).arg(model.code);}}
append(model);if(wasAtEnd){alertsDialogList.positionViewAtEnd();}}
function set(alertList){clear();for(var alert of alertList.values()){add(alert);}}}
delegate:NText{id:alertRecordText
...
ErrText{text:"Stop Condition: <font color=\"%1\">%2</font>".arg(Utility.getAppHexColor("lightText")).arg(state.stopConditionString)
visible:state.stopConditionString
color:Utility.getAppHexColor("normalText")}
ErrText{text:"Last Beep Reason: <font color=\"%1\">%2</font>".arg(Utility.getAppHexColor("lightText")).arg(state.beepReasonString)
visible:state.beepReasonString
color:Utility.getAppHexColor("normalText")}}}}
component ConfigDiff:ColumnLayout{id:configDiff
...
Behavior on animation{id:animBehavior
NumberAnimation{easing.type:Easing.InOutExpo
duration:400}}
function reset(){animBehavior.enabled=false;diffCheckbox.checked=false;animation=0;animBehavior.enabled=true;}
function showHideDiff(show){if(show){maxHeight=dialog.fullHeight-dialog.height;}
animation=show?1:0;}
function displayValue(item,value){var type=packageConfig.getParamType(item.name);if(type==="Double"||type==="Int"){return value.toString();}else if(type==="Bool"){return value?"On":"Off";}else if(type==="Enum"){return packageConfig.getEnumNames(item.name)[value];}else if(type==="String"){return value;}else if(type==="Bitfield"){return"[option set]";}
return"[unknown]";}
function setModel(m){var model=diffModel;model.clear();var empty=true;for(var item of m){item.value=displayValue(item,item.value);item.otherValue=displayValue(item,item.otherValue);model.append(item);empty=false;}
if(empty){diffCheckbox.checked=false;}}
CheckBox{id:diffCheckbox
implicitHeight:indicator.height
Layout.leftMargin:-4
visible:!isEmpty
checked:false
onCheckedChanged:{configDiff.showHideDiff(checked);}
contentItem:Item{implicitWidth:checkLabel.implicitWidth+22
...
scale:0.707
color:Utility.getAppHexColor("lightText")}}}}
Item{implicitHeight:configDiff.autoHeight
Layout.fillWidth:true
clip:true
Row{id:diffHeader
anchors.horizontalCenter:parent.horizontalCenter
NText{width:diffView.width-2*valueColumnWidth-10
horizontalAlignment:Text.AlignLeft
verticalAlignment:Text.AlignVCenter
text:"Option"}
NText{width:valueColumnWidth
...
delegate:Rectangle{color:index%2===0?Utility.getAppHexColor("lightBackground"):Utility.getAppHexColor("normalBackground")
...
BaseDialog{id:applyConfigDialog
parent:ApplicationWindow.overlay
...
Rectangle{id:welcomeDialog
anchors.fill:parent
z:100
visible:false
color:"#AA000000"
function show(){visible=true;}
Page{title:"Welcome to Float Out Boy"
anchors.centerIn:parent
...
font.bold:true
padding:12}
footer:DialogButtonBox{standardButtons:Dialog.Yes|Dialog.No
onAccepted:{welcomeDialog.visible=false;settingsDialog.show(1);}
onRejected:{welcomeDialog.visible=false;}}
ColumnLayout{id:welcomeDialogContent
anchors.fill:parent
LText{id:welcomeMessage
Layout.fillWidth:true
wrapMode:Text.WordWrap
text:"If you haven't configured your board yet, make sure to configure your motor and IMU before you try to ride the board.\n\nTo run the motor wizard, you need to disable the Float Out Boy package first. You can do so in Float Out Boy Settings > Setup, which you can access using the cog button in the top right corner of the main Float Out Boy screen.\n\nAfter the setup, you can safely verify your board balances correctly using the Handtest mode.\n\nTo configure Float Out Boy, you need to set the High and Low Voltage Thresholds in Float Out Boy Cfg > Specs according to your battery voltage. The rest of the configuration options has defaults that should provide you with a well-behaving, rideable board.\n\nHappy shredding!\n\nGo to Settings > Setup now?"}
CheckBox{id:welcomeShowAgain
checked:!preferences.showWelcomeDialog
text:"Don't show again"
onCheckedChanged:{preferences.showWelcomeDialog=!checked}}}}}
Rectangle{id:uiHintsRect
anchors.fill:parent
z:90
visible:uiHintsSwipe.count>0
color:"#CC000000"
SwipeView{id:uiHintsSwipe
anchors.fill:parent
clip:true
property bool seenAll
onCurrentIndexChanged:{if(currentIndex>0){itemAt(currentIndex-1).shown=true;}
if(currentIndex==count-1){seenAll=true;}}
function clear(){for(let i=count-1;i>=0;i--){takeItem(i);}}
function markAllShown(){for(let i=0;i<count;i++){itemAt(i).shown=true;}}}
NText{anchors.horizontalCenter:parent.horizontalCenter
anchors.top:parent.top
anchors.margins:20
font.pixelSize:24
text:"Tips & Tricks"}
PageIndicator{anchors.horizontalCenter:parent.horizontalCenter
...
text:"Skip now"
onClicked:{uiHintsSwipe.clear();}}
Button{id:uiHintsClose
...
onClicked:{uiHintsSwipe.markAllShown();uiHintsSwipe.clear();}}
function fill(doDataRecord=false){uiHints.fill(uiHintsSwipe);if(doDataRecord){uiHints.fillDataRecord(uiHintsSwipe);}
uiHintsSwipe.seenAll=uiHintsSwipe.currentIndex==uiHintsSwipe.count-1;}
function fillDataRecord(){uiHints.fillDataRecord(uiHintsSwipe);uiHintsSwipe.seenAll=uiHintsSwipe.currentIndex==uiHintsSwipe.count-1;}
Component.onCompleted:{fill();}
Connections{target:state
function onCommsInitialized(){uiHintsRect.fillDataRecord();}}}
OverlayPlot{id:recordedPlot
maxWindowMinutes:preferences.plotMaxWindowMinutes
plotData:RecordedPlotData{id:recordedPlotData
seriesMetaData:plotSeriesMetaData
seriesExtraColors:plotSeriesExtraColors}
function show(){init(state.ticksPerSecond);plotData.fetch(state.ticksPerSecond);visible=true;}
...
duration:200}}
LText{id:batteryBarValue
anchors.centerIn:parent
z:2
font.pixelSize:parent.height*0.7
text:Math.round(parent.value)+"%"}
Rectangle{id:batteryBarFill
width:parent.width*clamp(parent.value/100,0,1)
height:parent.height
z:1
color:Utility.getAppHexColor("lightAccent")}
LText{id:batteryBarVoltageValue
anchors.horizontalCenter:parent.horizontalCenter
...
z:2
font.pixelSize:fontSizeNormal
text:{var voltageString="";if(preferences.showBattVoltage){voltageString="%1V".arg(toFixed1Zero(parent.voltage));}
if(preferences.showBattVoltagePerCell){if(preferences.showBattVoltage){voltageString+=" / ";}
voltageString+="%1V".arg(toFixed2Zero(parent.voltage/motorConfig.batteryCells));}
return voltageString;}}
Item{id:chargingInfo
anchors.centerIn:parent
anchors.horizontalCenterOffset:parent.width/4
...
font.pixelSize:fontSizeSmall
text:"charging"}
LText{anchors.bottom:parent.bottom
...
clip:true
text:nameOrId
function load(){var boardName=board.getName();name=boardName;nameOrId=boardName??board.getId();}
Component.onCompleted:load();}
ToolButton{id:editBoardNameButton
Layout.alignment:Qt.AlignVCenter
...
opacity:0
text:"WHEELSLIP"
states:State{name:"shown";when:state.wheelslip
PropertyChanges{target:speedDialWheelslip;opacity:1;}}
transitions:[Transition{to:"shown"
PropertyAnimation{target:speedDialWheelslip;property:"opacity";duration:200;easing.type:Easing.OutExpo;}},Transition{from:"shown"
PropertyAnimation{target:speedDialWheelslip;property:"opacity";duration:1000;easing.type:Easing.OutExpo;}}]}
Glow{anchors.fill:speedDialWheelslip
...
Item{anchors.top:speedDialFrame.bottom
anchors.topMargin:-speedDialFrame.width*0.125
width:parent.width
height:1.8*unit
NText{id:currentDialLabel
anchors.bottom:parent.bottom
width:parent.height
font.pixelSize:fontSizeSmall
horizontalAlignment:Text.AlignHCenter
text:"current"}
Dial{anchors.bottom:currentDialLabel.top
...
width:unit
height:0.8*unit
readonly property real alpha:0.05;property real value:0;LText{width:parent.width
font.pixelSize:hudLeft.fontSizeBars
horizontalAlignment:Text.AlignHCenter
text:(toFixed1Zero(parent.value))}
DText{width:parent.width
anchors.bottom:parent.bottom
font.pixelSize:fontSizeSmall
horizontalAlignment:Text.AlignHCenter
...
anchors.bottom:parent.bottom
width:parent.width
NText{id:batteryCurrentLabel
anchors.bottom:parent.bottom
anchors.horizontalCenter:parent.horizontalCenter
font.pixelSize:fontSizeSmall
text:"battery current"}
Rectangle{id:batteryCurrentBar
...
width:parent.height
height:width
property real leftVoltage:state.rtData["footpad.adc_left"]
property real rightVoltage:state.rtData["footpad.adc_right"]
...
contextType:"2d"}
component SensorCanvas:FpCanvas{property Path path
property bool on:false
opacity:on?1:0
onOnChanged:requestPaint();Behavior on opacity{NumberAnimation{easing.type:Easing.OutCubic
duration:60}}
onPaint:{var context=getContext("2d");context.reset();context.fillStyle=Utility.getAppHexColor("lightAccent")
context.path=path
context.fill()}}
FpCanvas{id:footpadCanvas
onPaint:{var context=getContext("2d");context.reset();context.strokeStyle=Utility.getAppHexColor("lightText")
context.lineWidth=2
context.path=footpadPath
context.stroke()}}
SensorCanvas{id:leftSensor
path:footpadLeftSensorPath}
Glow{anchors.fill:leftSensor
...
color:Utility.getAppHexColor("lightAccent")
source:leftSensor}
SensorCanvas{id:rightSensor
path:footpadRightSensorPath}
Glow{anchors.fill:rightSensor
radius:Math.round(0.08*unit)
samples:radius*2+1
spread:0.6
...
onValueChanged:rollCanvas.requestPaint();Behavior on value{id:rollEasingBehavior
NumberAnimation{easing.type:Easing.OutCirc
duration:100}}
...
onPaint:{var context=getContext("2d");context.reset();context.translate(width/2,height/2);context.rotate(Math.PI/180*parent.value);context.path=rollPath;context.lineWidth=2;context.strokeStyle=Utility.getAppHexColor("lightText")
context.stroke();}}
NText{id:rollText
anchors.bottom:parent.bottom
anchors.horizontalCenter:parent.horizontalCenter
...
transitions:[Transition{to:"running"
PropertyAnimation{target:pitch;property:"runtimeOpacity";duration:200;easing.type:Easing.OutExpo;}},Transition{from:"running"
PropertyAnimation{target:pitch;property:"runtimeOpacity";duration:2000;easing.type:Easing.OutExpo;}}]
Behavior on pitchValue{NumberAnimation{easing.type:Easing.OutCirc
duration:100}}
Behavior on balancePitchValue{NumberAnimation{easing.type:Easing.OutCirc
duration:100}}
Behavior on setpointValue{NumberAnimation{easing.type:Easing.OutCirc
duration:100}}
...
Path{id:pitchSetpointPath
scale:Qt.size(pitchCanvas.scale,pitchCanvas.scale)
PathSvg{path:"M -86 0 L -43 0 M 43 0 L 86 0"}}
function paintPath(context,paths,rotation,color){context.save();context.translate(width/2,height/2);context.rotate(Math.PI/180*rotation);context.lineWidth=2;context.strokeStyle=Utility.getAppHexColor(color);for(let path of paths){context.path=path;context.stroke();}
context.restore();}
onPaint:{var context=getContext("2d");context.reset()
paintPath(context,[pitchBoardPath],parent.balancePitchValue,"lightAccent");paintPath(context,[pitchBoardPath,pitchWheelPath],parent.pitchValue,"lightText");context.globalAlpha=parent.runtimeOpacity;paintPath(context,[pitchSetpointPath],parent.setpointValue,"orange");}}
Text{id:pitchPushbackText
anchors.top:parent.top
anchors.left:parent.left
anchors.leftMargin:0.17*unit
property string satText:state.setpointAdjustmentTypeString
onSatTextChanged:{if(satText){text=satText;if(state.setpointAdjustmentClass==state.sac_Error){color=Utility.getAppHexColor("red");}else if(state.setpointAdjustmentClass==state.sac_Warning){color=Utility.getAppHexColor("orange");}else{color=Utility.getAppHexColor("normalText");}}}
font.pixelSize:fontSizeSmall
opacity:0
states:State{name:"shown";when:pitchPushbackText.satText!=""&&state.pkgState===state.s_Running
PropertyChanges{target:pitchPushbackText;opacity:1;}}
transitions:[Transition{to:"shown"
PropertyAnimation{target:pitchPushbackText;property:"opacity";duration:200;easing.type:Easing.OutExpo;}},Transition{from:"shown"
PropertyAnimation{target:pitchPushbackText;property:"opacity";duration:2000;easing.type:Easing.OutExpo;}}]}
NText{id:pitchPitchLabel
anchors.bottom:parent.bottom
anchors.left:parent.left
...
horizontalAlignment:Text.AlignHCenter
text:"balance"}
Text{id:pitchBalancePitchValue
...
component VerticalValue:Item{height:parent.verticalValueItemHeight
...
NText{anchors.bottom:parent.bottom
width:parent.width
font.pixelSize:fontSizeSmall
horizontalAlignment:Text.AlignHCenter
text:labelText}
...
globState:state}
VerticalValue{id:odometer
anchors.right:parent.right
labelText:"life"
decimals:0
unit:vescConfig.distanceUnit}
VerticalValue{id:tachometer
anchors.right:odometer.left
labelText:"trip"
unit:vescConfig.distanceUnit}}}
TabBar{id:bottomStackTabs
Layout.fillWidth:true
clip:true
TabButton{text:"Tunes"}
TabButton{text:"Control"}
TabButton{text:"Data"}
background:Rectangle{color:Utility.getAppHexColor("lightBackground");}
onCurrentIndexChanged:{bottomStackLayout.currentIndex=currentIndex;}}
StackLayout{id:bottomStackLayout
Layout.fillWidth:true
Layout.fillHeight:true
clip:true
Item{id:tunesPage
Layout.fillWidth:true
Layout.fillHeight:true
GridLayout{property int spacing:0.6*unit
anchors.fill:parent
...
Repeater{model:preferences.tuneSlotCount
Button{id:button
Layout.fillWidth:true
Layout.fillHeight:true
...
topInset:0
bottomInset:0
onPressAndHold:{editTuneDialog.show(slot,tuneName);}
onClicked:{if(tuneName){var tune=tuneManager.loadTune(slot);packageConfig.applyConfig(tune);}}
...
visible:lights.enabled
Button{Layout.preferredWidth:3*unit
Layout.alignment:Qt.AlignHCenter
text:lights.on?"Lights Off":"Lights On"
onClicked:{commands.sendLightsControl(!lights.on);}}
Button{id:headlightsButton
...
falseIconPath:Path{PathSvg{path:"M 10 12 L 10 38 L 42 38 M 10 30 L 22 22 L 32 28 L 39 18"}}}
ScrollView{anchors.fill:parent
contentWidth:availableWidth
clip:true
...
component Value:Item{implicitHeight:label.implicitHeight
...
LText{anchors.left:label.right
anchors.top:parent.top
font.pixelSize:fontSizeNormal
...
plotData:PlotData{id:rtPlotData
seriesMetaData:plotSeriesMetaData
seriesExtraColors:plotSeriesExtraColors}
Connections{target:state
function onCommsInitialized(){rtPlotData.init(state.rtDataAllItems,state.ticksPerSecond);rtPlot.init(state.ticksPerSecond);}}}}}
NText{id:bottomLine
Layout.fillWidth:true
