...
use super::FloatOutBoyAllDataPayloads;
...
pub(super) fn float_out_boy_append_all_data_mode2<const N: usize>(
    ...
    packet.push_float32_auto(payloads.distance_abs().distance().as_meters());
    ...
    packet.push(payloads.battery_temperature().map_or(0, |temp| {
...
pub(super) fn float_out_boy_append_all_data_mode3<const N: usize>(
    ...
    packet.push_scaled_i16(payloads.discharged_charge().charge().as_amp_hours(), 10.0);
    packet.push_scaled_i16(payloads.charged_charge().charge().as_amp_hours(), 10.0);
    packet.push_scaled_i16(payloads.discharged_energy().energy().as_watt_hours(), 1.0);
    packet.push_scaled_i16(payloads.charged_energy().energy().as_watt_hours(), 1.0);
    packet.push(float_out_boy_scaled_u8(
        payloads.battery_level().as_fraction().min(1.25),
        200.0,
    ));
}
...
pub(super) fn float_out_boy_append_all_data_mode4<const N: usize>(
    ...
    packet.push_scaled_i16(payloads.charging_current().current().as_amps(), 10.0);
    packet.push_scaled_i16(payloads.charging_voltage().voltage().as_volts(), 10.0);
}
