...
fn app_data_running_clamps_angle_i_at_default_ki_limit_like_float_out_boy_pid() {
    ...
    let base = payloads.base();
    let setpoint = FloatOutBoyRealtimeRuntimeSetpoint::new(AngleDegrees::from_degrees(10_000.0));
    let setpoints = FloatOutBoyRealtimeRuntimeSetpoints::new(
        setpoint, setpoint, setpoint, setpoint, setpoint, setpoint,
    );
    let base = FloatOutBoyAllDataBasePayload::new(
...
fn app_data_running_limits_handtest_and_flywheel_current_like_float_out_boy_loop() {
    ...
    for (mode, expected_current) in [
        ...
        let base = payloads.base();
        let setpoint = FloatOutBoyRealtimeRuntimeSetpoint::new(AngleDegrees::from_degrees(10.0));
        let setpoints = FloatOutBoyRealtimeRuntimeSetpoints::new(
            setpoint, setpoint, setpoint, setpoint, setpoint, setpoint,
        );
...
fn app_data_normal_algorithm_trace_matches_float_out_boy_loop_order() {
    ...
    let base = payloads.base();
    let stopped_base = FloatOutBoyAllDataBasePayload::new(
        FloatOutBoyRealtimeBalanceCurrent::new(MotorCurrent::new(Current::from_amps(0.0))),
        base.attitude(),
        base.status(),
        base.footpad(),
    ...
    // `third_party/float-out-boy/src/main.c:263-270`; `reset_runtime_vars(d)`
    // seeds only the board setpoint from balance pitch at
    // `third_party/float-out-boy/src/main.c:239-252`, and READY breaks before
    // RUNNING PID in `third_party/float-out-boy/src/main.c:1018-1037`.
    assert_eq!(engaged_ride_state.run_state(), FloatOutBoyRunState::Running);
    assert_eq!(engaged_ride_state.mode(), FloatOutBoyMode::Normal);
    assert_eq!(
    ...
    // `third_party/float-out-boy/src/main.c:918-956` before requesting motor
    // current. Raw pitch equals the centered board setpoint here, so the
    // booster proportional is zero by `third_party/float-out-boy/src/main.c:921-922`.
    assert!(
    ...
    // at `third_party/float-out-boy/src/main.c:1075-1079`; requested current
    // takes the current-control branch at
    // `third_party/float-out-boy/src/motor_control.c:92-99`.
