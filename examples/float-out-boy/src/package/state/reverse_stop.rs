...
use vescpkg_rs::prelude::{
    AngleDegrees, Distance, Frequency, Ratio, Rpm, SampleRate, SignedRatio, SignedTripDistance,
...
impl ReverseStop {
    ...
    pub(super) fn update(
        ...
        if (!enabled || erpm > ENTRY_ERPM) && self.progress == COMPLETE {
            self.start_distance = distance.distance();
            return;
        }
        ...
        let distance_diff = new_distance
            ...
            .scaled_by(source_sign(self.target_distance).as_ratio());
        ...
        if self.progress == COMPLETE {
            if distance_diff.as_meters() > 0.0 {
                self.start_distance = distance.distance();
            }
            return;
        }
        ...
        let target_progress = self.current_distance.as_meters() / self.target_distance.as_meters();
...
fn source_sign(value: Distance) -> SignedRatio {
    SignedRatio::from_ratio_const(if value.is_negative() { -1.0 } else { 1.0 })
}
