...
#[test]
fn startup_ready_above_low_voltage_margin_schedules_one_long_beep_like_float_out_boy() {
    let telemetry = FirmwareTest::new()
...
fn startup_ready_below_low_voltage_margin_reports_low_battery_and_beeps_twice() {
    ...
    assert_eq!(
        ...
        FloatOutBoyBeepReason::LowBattery
    );
...
fn startup_ready_gate_does_not_engage_active_footpads_until_next_loop() {
    ...
    // `third_party/float-out-boy/src/main.c:833-852`; READY engagement cannot run
...
#[test]
fn startup_ready_resets_runtime_vars_like_float_out_boy() {
    let app_data = TimestampTicks::from_ticks(0);
    let telemetry = FirmwareTest::new();
    telemetry.set_imu_ready(true);
    telemetry.set_imu_attitude(
    ...
    // Float Out Boy calls `reset_runtime_vars(d)` before READY at
    // `third_party/float-out-boy/src/main.c:833-837`; reset clears
    // `balance_current` at `third_party/float-out-boy/src/main.c:246`, resets
...
#[test]
fn disabled_config_applies_before_startup_ready_like_float_out_boy() {
    let app_data = TimestampTicks::from_ticks(0);
    let telemetry = FirmwareTest::new();
    telemetry.set_imu_ready(true);
    let imu = telemetry.imu();
    ...
    // startup gate at `third_party/float-out-boy/src/main.c:184-190`; `state_set_disabled` forces
    // `STATE_DISABLED` at `third_party/float-out-boy/src/state.c:41-47`, so `third_party/float-out-boy/src/main.c:833-838`
...
fn ready_engage_resets_runtime_vars_like_float_out_boy() {
    ...
    // `state_engage(d)` at `third_party/float-out-boy/src/main.c:263-270`, then
...
#[test]
fn ready_normal_charging_does_not_engage_like_float_out_boy_can_engage() {
    let app_data = TimestampTicks::from_ticks(0);
    let telemetry = FirmwareTest::new();
    telemetry.set_imu_ready(true);
    let imu = telemetry.imu();
...
#[test]
fn motor_payload_refreshes_like_float_out_boy_motor_data_update() {
    let app_data = TimestampTicks::from_ticks(0);
    let telemetry = FirmwareTest::new()
        .with_runtime_motor(
...
#[test]
fn foc_id_current_refreshes_like_float_out_boy_all_data() {
    // Float Out Boy v1.2.1 encodes `fabsf(VESC_IF->foc_get_id()) * 3` for
    // compact all-data at `third_party/float-out-boy/src/main.c:1364-1368`.
...
fn running_runtime_fixture() -> (TimestampTicks, FirmwareTest, FloatOutBoyPackageState) {
    // C map: this fixture shapes the RUNNING loop's cached runtime state the
    // same way Float Out Boy refreshes it before balance-current and motor-control math.
    let now = TimestampTicks::from_ticks(0);
    let telemetry = FirmwareTest::new();
    telemetry.set_imu_ready(true);
    let payloads = sample_all_data_payloads_with_ride_state(
    ...
    let base = FloatOutBoyAllDataBasePayload::new(
        FloatOutBoyRealtimeBalanceCurrent::new(MotorCurrent::new(Current::from_amps(4.75))),
        FloatOutBoyAllDataAttitude::new(
            FloatOutBoyRealtimeBalancePitch::new(AngleRadians::from_degrees(1.0)),
            base.attitude().roll(),
            base.attitude().pitch(),
        ),
        base.status(),
...
fn ready_bms_fixture() -> (FirmwareTest, FloatOutBoyPackageState) {
    let telemetry = FirmwareTest::new();
    telemetry.set_imu_ready(true);
    let payloads = sample_all_data_payloads_with_ride_state(
        FloatOutBoyRunState::Ready,
...
fn running_reverse_stop_fixture(
    ...
    motor_erpm: Rpm,
) -> (TimestampTicks, FirmwareTest, FloatOutBoyPackageState) {
    let now = TimestampTicks::from_ticks(0);
    ...
    let setpoints = FloatOutBoyRealtimeRuntimeSetpoints::new(
        FloatOutBoyRealtimeRuntimeSetpoint::new(board_setpoint),
        base.setpoints().atr(),
        base.setpoints().brake_tilt(),
        base.setpoints().torque_tilt(),
        base.setpoints().turn_tilt(),
...
#[test]
fn running_enters_reverse_stop_from_reverse_motor_speed_like_float_out_boy() {
    let app_data = TimestampTicks::from_ticks(0);
    let telemetry = FirmwareTest::new().with_runtime_motor(
...
#[test]
fn running_reverse_stop_carries_only_error_pushbacks_into_erpm_like_float_out_boy() {
    let cases = [
        (
        ...
        (FloatOutBoySetpointAdjustment::PushbackDuty, Rpm::ZERO),
    ];
    ...
    for (adjustment, expected_total_erpm) in cases {
        ...
        // Float Out Boy carries only SAT values at or above `SAT_PB_HIGH_VOLTAGE`
        // into reverse-stop at `third_party/float-out-boy/src/main.c:538-550`.
...
#[test]
fn running_reverse_stop_grows_target_past_erpm_tolerance_like_float_out_boy() {
    ...
    // at `third_party/float-out-boy/src/main.c:522-529`.
    assert_eq!(
...
#[test]
fn running_reverse_stop_exits_below_half_tolerance_while_moving_forward_like_float_out_boy() {
    let (app_data, telemetry, mut state) = running_reverse_stop_fixture(
    ...
    // `third_party/float-out-boy/src/main.c:530-536`.
    assert_eq!(
        state
            .all_data_payloads()
            .base()
...
#[test]
fn running_reverse_stop_stays_active_at_half_tolerance_while_reversing_like_float_out_boy() {
    let (app_data, telemetry, mut state) = running_reverse_stop_fixture(
...
fn running_protective_pushback_fixture(
    ...
    motor_erpm: Rpm,
    adjustment: FloatOutBoySetpointAdjustment,
    ...
    let motor = crate::domain::FloatOutBoyAllDataMotorPayload::new(
    ...
    let status = FloatOutBoyAllDataStatus::new(
        base.status()
            .ride_state()
    ...
    let board_setpoint = match adjustment {
        FloatOutBoySetpointAdjustment::Centering => AngleDegrees::from_degrees(1.0),
        FloatOutBoySetpointAdjustment::PushbackDuty => AngleDegrees::from_degrees(5.0),
        FloatOutBoySetpointAdjustment::PushbackHighVoltage
        | FloatOutBoySetpointAdjustment::PushbackLowVoltage
        | FloatOutBoySetpointAdjustment::PushbackTemperature => AngleDegrees::from_degrees(8.0),
        _ => AngleDegrees::ZERO,
    };
    ...
    let setpoints = base
        .setpoints()
        .with_board(FloatOutBoyRealtimeRuntimeSetpoint::new(board_setpoint));
    let base = FloatOutBoyAllDataBasePayload::new(
        base.balance_current(),
        base.attitude(),
        status,
...
#[test]
fn controller_input_selects_connected_uart_or_ppm_and_applies_deadband_like_float_out_boy() {
...
#[test]
fn running_nose_angling_changes_the_production_setpoint_like_float_out_boy() {
    let (now, telemetry, mut state) = running_speed_or_sag_fixture(
        InputVoltage::new(Voltage::from_volts(72.0)),
        DirectionalMotorCurrent::new(Current::ZERO),
        VehicleSpeed::new(Speed::ZERO),
...
#[test]
fn running_torque_and_atr_update_runtime_setpoints_like_float_out_boy() {
    let (now, telemetry, mut state) = running_speed_or_sag_fixture(
        InputVoltage::new(Voltage::from_volts(72.0)),
...
fn set_protective_ride_state(
    ...
    let footpad = if matches!(mode, FloatOutBoyMode::Flywheel) {
        FloatOutBoyFootpadSample::new(Voltage::ZERO, Voltage::ZERO, FloatOutBoyFootpadState::None)
    } else {
        base.footpad()
    };
...
fn enable_beeper(state: &mut FloatOutBoyPackageState) {
    assert!(state.serialized_config.editor().set_beeper_enabled(true));
    state.refresh_config_runtime_state();
}

fn first_beeper_high_tick(state: &mut FloatOutBoyPackageState, limit: usize) -> Option<usize> {
    (1..=limit).find(|_| matches!(state.tick_beeper(), Some(FloatOutBoyBeeperLevel::High)))
}

#[test]
fn running_temperature_warning_uses_float_out_boy_margin_priority_and_long_alert() {
    let cases = [
        (
...
fn record_bms_sample(
    ...
    state.record_bms_sample(FloatOutBoyBmsSample::new(
        cell_low_voltage,
        cell_high_voltage,
...
#[test]
fn running_enters_duty_pushback_above_configured_threshold_like_float_out_boy() {
    let (app_data, telemetry, mut state) = running_protective_pushback_fixture(
    ...
    // Default Float Out Boy duty pushback targets 5 degrees above 0.80 duty and
...
#[test]
fn running_enters_pack_low_voltage_pushback_below_sag_allowance_like_float_out_boy() {
    let (now, telemetry, mut state) = running_speed_or_sag_fixture(
...
#[test]
fn running_enters_speed_pushback_above_configured_threshold_like_float_out_boy() {
    let (now, telemetry, mut state) = running_speed_or_sag_fixture(
        InputVoltage::new(Voltage::from_volts(72.0)),
        DirectionalMotorCurrent::new(Current::ZERO),
    ...
    assert_eq!(base.status().beep_reason(), FloatOutBoyBeepReason::Speed);
    assert_eq!(
        base.setpoints().board().angle(),
        state.runtime_duty_pushback_step()
    );
...
#[test]
fn running_pack_low_voltage_sag_boundaries_match_float_out_boy() {
    for (volts, amps, expected) in [
        (52.0, 40.0, FloatOutBoySetpointAdjustment::None),
        (53.0, 20.0, FloatOutBoySetpointAdjustment::None),
        (
            53.0,
            19.9,
            FloatOutBoySetpointAdjustment::PushbackLowVoltage,
        ),
        (53.9, 5.0, FloatOutBoySetpointAdjustment::None),
        (53.9, 4.9, FloatOutBoySetpointAdjustment::PushbackLowVoltage),
    ] {
        let (now, telemetry, mut state) = running_speed_or_sag_fixture(
...
#[test]
fn running_pack_sag_clears_existing_low_voltage_pushback_like_float_out_boy() {
...
#[test]
fn running_speed_pushback_boundaries_direction_and_disable_match_float_out_boy() {
    for (threshold, speed, expected, expected_angle) in [
        (10, 10.0, FloatOutBoySetpointAdjustment::None, 0.0),
        (10, 10.1, FloatOutBoySetpointAdjustment::PushbackSpeed, 1.0),
        (
            10,
            -10.1,
            FloatOutBoySetpointAdjustment::PushbackSpeed,
        ...
        (0, 20.0, FloatOutBoySetpointAdjustment::None, 0.0),
    ] {
        let (now, telemetry, mut state) = running_speed_or_sag_fixture(
            InputVoltage::new(Voltage::from_volts(72.0)),
            DirectionalMotorCurrent::new(Current::ZERO),
...
#[test]
fn running_protective_priority_is_duty_then_pack_voltage_then_speed_like_float_out_boy() {
    for (duty, voltage, expected) in [
        (0.81, 51.0, FloatOutBoySetpointAdjustment::PushbackDuty),
        (
            0.10,
            51.0,
            FloatOutBoySetpointAdjustment::PushbackLowVoltage,
...
#[test]
fn running_wheelslip_refreshes_timer_and_zeros_target_above_max_duty_like_float_out_boy() {
...
#[test]
fn running_wheelslip_exit_uses_strict_timer_and_raw_duty_thresholds_like_float_out_boy() {
    let (_, telemetry, mut state) = running_protective_pushback_fixture(
...
#[test]
fn reverse_stop_entry_precedes_wheelslip_detection_like_float_out_boy() {
    let (now, telemetry, mut state) = running_protective_pushback_fixture(
...
#[test]
fn running_duty_at_threshold_does_not_push_back_like_float_out_boy() {
    let (app_data, telemetry, mut state) = running_protective_pushback_fixture(
...
#[test]
fn running_duty_pushback_clears_below_threshold_like_float_out_boy() {
    let (app_data, telemetry, mut state) = running_protective_pushback_fixture(
...
#[test]
fn running_enters_high_voltage_pushback_one_volt_above_threshold_like_float_out_boy() {
    let (app_data, telemetry, mut state) = running_protective_pushback_fixture(
        SignedRatio::from_ratio_const(0.10),
        Rpm::from_revolutions_per_minute(1_000.0),
        FloatOutBoySetpointAdjustment::None,
...
#[test]
fn bms_cell_over_voltage_enters_immediate_high_voltage_pushback_like_float_out_boy() {
...
#[test]
fn bms_connection_fault_uses_high_voltage_angle_and_error_pushback_like_float_out_boy() {
    let (_, telemetry, mut state) = running_protective_pushback_fixture(
        SignedRatio::from_ratio_const(0.10),
        Rpm::from_revolutions_per_minute(1_000.0),
        FloatOutBoySetpointAdjustment::None,
...
fn bms_temperature_faults_use_low_voltage_angle_and_source_reason_order() {
    let cases = [
        (
            ...
            FloatOutBoyBmsTemperature::from_degrees_celsius(35),
            FloatOutBoyBeepReason::CellOverTemperature,
        ),
        (
            FloatOutBoyBmsTemperature::from_degrees_celsius(-1),
            FloatOutBoyBmsTemperature::from_degrees_celsius(30),
            FloatOutBoyBmsTemperature::from_degrees_celsius(35),
            FloatOutBoyBeepReason::CellUnderTemperature,
        ),
        (
            FloatOutBoyBmsTemperature::from_degrees_celsius(20),
            FloatOutBoyBmsTemperature::from_degrees_celsius(30),
            FloatOutBoyBmsTemperature::from_degrees_celsius(61),
            FloatOutBoyBeepReason::BmsOverTemperature,
        ),
    ];
...
#[test]
fn bms_cell_under_voltage_bypasses_pack_sag_checks_like_float_out_boy() {
    let (_, telemetry, mut state) = running_protective_pushback_fixture(
        SignedRatio::from_ratio_const(0.10),
        Rpm::from_revolutions_per_minute(1_000.0),
        FloatOutBoySetpointAdjustment::None,
...
#[test]
fn ready_bms_connection_alert_uses_strict_fifteen_second_timer_like_float_out_boy() {
...
#[test]
fn ready_bms_connection_alert_schedules_four_short_beeps_like_float_out_boy() {
    let (telemetry, mut state) = ready_bms_fixture();
    enable_bms(&mut state);
    assert!(state.serialized_config.editor().set_beeper_enabled(true));
    state.refresh_config_runtime_state();
...
#[test]
fn ready_bms_cell_balance_alert_requires_disengage_and_alert_delays_like_float_out_boy() {
    let (telemetry, mut state) = ready_bms_fixture();
    enable_bms(&mut state);
    record_bms_sample(
        &mut state,
...
#[test]
fn ready_idle_nag_waits_for_stable_voltage_and_beeps_every_minute_like_float_out_boy() {
    let (telemetry, mut state) = ready_bms_fixture();
    enable_beeper(&mut state);
    let imu = telemetry.imu();
...
#[test]
fn running_high_voltage_pushback_uses_strict_half_second_delay_like_float_out_boy() {
    let (_, telemetry, mut state) = running_protective_pushback_fixture(
        SignedRatio::from_ratio_const(0.10),
        Rpm::from_revolutions_per_minute(1_000.0),
        FloatOutBoySetpointAdjustment::None,
...
#[test]
fn running_low_voltage_refreshes_high_voltage_timer_before_every_selection_branch_like_float_out_boy()
 {
    let cases = [
        (
            FloatOutBoyMode::Normal,
...
#[test]
fn running_high_voltage_pushback_uses_negative_target_for_reverse_erpm_like_float_out_boy() {
    let (app_data, telemetry, mut state) = running_protective_pushback_fixture(
        SignedRatio::from_ratio_const(0.10),
        Rpm::from_revolutions_per_minute(-1_000.0),
        FloatOutBoySetpointAdjustment::None,
...
#[test]
fn running_duty_pushback_precedes_high_voltage_like_float_out_boy() {
    let (app_data, telemetry, mut state) = running_protective_pushback_fixture(
        SignedRatio::from_ratio_const(0.81),
        Rpm::from_revolutions_per_minute(1_000.0),
        FloatOutBoySetpointAdjustment::None,
...
#[test]
fn running_runtime_requests_balance_current_like_float_out_boy_loop() {
    ...
    // via `motor_control_request_current` at `third_party/float-out-boy/src/main.c:949-956`.
    assert_eq!(
...
#[test]
fn running_motor_apply_uses_current_branch_like_float_out_boy_loop() {
    let (app_data, telemetry, mut state) = running_runtime_fixture();
    let imu = telemetry.imu();
    let bindings = telemetry.motor();
    ...
    // `third_party/float-out-boy/src/main.c:918-956`, then `float_out_boy_thd` calls
    // `motor_control_apply` at `third_party/float-out-boy/src/main.c:1076`; a
    // current request takes the `mc_set_current` branch at
    // `third_party/float-out-boy/src/motor_control.c:92-121`.
