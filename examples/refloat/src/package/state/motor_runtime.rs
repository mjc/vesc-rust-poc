...
pub(super) struct RefloatMotorCurrentFilter {
    a0: f32,
    a1: f32,
    a2: f32,
    b1: f32,
...
impl RefloatMotorCurrentFilter {
    ...
    fn process(&mut self, current: DirectionalMotorCurrent) -> RefloatRealtimeFilteredMotorCurrent {
        ...
        RefloatRealtimeFilteredMotorCurrent::new(DirectionalMotorCurrent::new(Current::from_amps(
            output,
        )))
    }
}
...
mod tests {
    ...
    #[test]
    fn disabled_current_filter_returns_directional_current_like_refloat() {
        let mut filter = RefloatMotorCurrentFilter::source_startup();
