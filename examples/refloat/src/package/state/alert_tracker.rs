use crate::domain::{RefloatAlertId, RefloatFatalErrorState, RefloatRealtimeAlertMask};
...
impl Default for AlertTrackerState {
    fn default() -> Self {
        Self {
            active_alerts: RefloatRealtimeAlertMask::empty(),
...
impl AlertTrackerState {
    pub(super) fn update_firmware_fault(
        ...
        self.fatal_error = match (is_active, persistent_fatal_error, self.fatal_error) {
            (true, _, _) | (false, true, RefloatFatalErrorState::Present) => {
                RefloatFatalErrorState::Present
            }
            _ => RefloatFatalErrorState::None,
        };
    }

    pub(super) fn clear_fatal(&mut self) {
        self.fatal_error = RefloatFatalErrorState::None;
    }

    pub(super) const fn active_alerts(&self) -> RefloatRealtimeAlertMask {
        self.active_alerts
    }
    ...
    pub(super) const fn fatal_error(&self) -> RefloatFatalErrorState {
        self.fatal_error
    }
...
mod tests {
    ...
    fn persistent_fatal_survives_fault_clear_until_control_clear() {
        ...
        assert_eq!(tracker.fatal_error(), RefloatFatalErrorState::Present);

        tracker.clear_fatal();
        assert_eq!(tracker.fatal_error(), RefloatFatalErrorState::None);
