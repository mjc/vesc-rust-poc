use crate::config::RefloatRemoteThrottleConfig;
...
fn zero_motor_current() -> MotorCurrent {
    ...
    // by writing zero at `third_party/refloat/src/main.c:239-252` and
...
impl CurrentSmoothing {
    ...
    // at `third_party/refloat/src/main.c:275-286` and `third_party/refloat/src/main.c:291-298`.
...
impl RemoteCurrentTarget {
    ...
    fn motor_current(self) -> MotorCurrent {
        ...
        // `third_party/refloat/src/main.c:1747-1756`; `do_rc_move` requests amps.
    ...
    fn halve(&mut self) {
        ...
        // `third_party/refloat/src/main.c:281-284`.
        self.0 /= 2;
    }
}
...
impl RemoteMove {
    ...
    pub(super) fn from_refloat_command(direction: u8, current: u8, time: u8, sum: u8) -> Self {
        // C map: `cmd_rc_move` treats checksum failure as `current = 0`, then
        // stores READY-state RC move fields at `third_party/refloat/src/main.c:1735-1758`.
    ...
    fn new(target: RemoteCurrentTarget, time: u8) -> Self {
        ...
        // `third_party/refloat/src/main.c:1735-1758`.
        match target {
            ...
            target if target.exceeds_packet_limit() => Self {
                ...
                // at `third_party/refloat/src/main.c:1753-1757`.
...
impl Default for RemoteControlState {
    fn default() -> Self {
        // C map: Refloat resets RC move state and current to zero at
...
impl RemoteControlState {
    ...
    pub(super) fn queue_move(&mut self, remote_move: RemoteMove) {
        ...
        // it at `third_party/refloat/src/main.c:1735-1758` and
        // `third_party/refloat/src/main.c:275-286`.
        self.counter = 0;
    ...
    fn request_active_move_current(&mut self, motor_erpm: Rpm) -> Option<MotorCurrent> {
        ...
        // `third_party/refloat/src/main.c:1069`, where active RC move steps
        // filter/request `rc_current` at `third_party/refloat/src/main.c:276-286`.
        self.filter_current(
    ...
    fn request_remote_throttle_current(
        ...
        if current_max <= Current::ZERO
            || !refloat_ticks_elapsed_seconds(system_time_ticks, disengage_ticks, grace_period)
            || input.abs() <= 0.02
        {
            self.current = zero_motor_current();
            return None;
        ...
        // Upstream READY falls through to `do_rc_move(d)` at
        // `third_party/refloat/src/main.c:1069`, where the remote-throttle idle
...
mod tests {
    ...
    #[test]
    fn remote_throttle_requests_idle_current_like_refloat_do_rc_move() {
        let mut remote_control = RemoteControlState::default();
        remote_control.set_input(RefloatRealtimeRemoteInput::new(
    ...
    #[test]
    fn rc_move_command_checksum_failure_becomes_zero_current_step_like_refloat() {
        // C map: `cmd_rc_move` compares `sum != time + current` as ints, then
        // sets `current = 0` at `third_party/refloat/src/main.c:1735-1741`.
        assert_eq!(
            RemoteMove::from_refloat_command(1, 1, 255, 0),
            RemoteMove {
                target: RemoteCurrentTarget::ZERO,
                duration_steps: 1
            }
    ...
    #[test]
    fn rc_move_command_steps_idle_current_like_refloat_do_rc_move() {
        let mut remote_control = RemoteControlState::default();
        remote_control.queue_move(RemoteMove::from_refloat_command(1, 40, 2, 42));
    ...
    #[test]
    fn rc_move_halves_large_target_after_500_steps_like_refloat_do_rc_move() {
        let mut remote_control = RemoteControlState::default();
        remote_control.queue_move(RemoteMove::from_refloat_command(1, 60, 6, 66));
