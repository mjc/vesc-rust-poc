...
pub(super) fn handle_runtime_tune_packet(state: &mut RefloatPackageState, bytes: &[u8]) -> bool {
    ...
    if let Some(brake) = payload.get(16) {
        let mut config = state.serialized_config.editor();
        let updated = [
            config.set_kp_brake(RefloatTuneNibble::low(*brake).brake_gain()),
            config.set_kp2_brake(RefloatTuneNibble::high(*brake).divided(10.0, 0.0, PidScale::new)),
        ]
        .into_iter()
        .all(core::convert::identity);
        debug_assert!(updated);
...
mod tests {
    ...
    fn tune_nibble_keeps_exact_endpoints_without_primitive_conversions() {
        ...
        assert_eq!(
            RefloatTuneNibble(9).divided(10.0, 0.0, RateCurrentGain::new),
            RateCurrentGain::new(9.0 / 10.0),
        );
        assert_eq!(
            RefloatTuneNibble(9).torque_tilt_strength(),
            PidScale::new((9.0 / 10.0) * 0.3),
        );
        assert_eq!(
            RefloatTuneNibble(6).brake_gain(),
