use crate::config::RefloatHapticConfig;
use crate::domain::{RefloatMode, RefloatRunState, RefloatSetpointAdjustment};
...
pub(super) struct HapticFeedbackInput {
    pub(super) run_state: RefloatRunState,
    pub(super) mode: RefloatMode,
    pub(super) setpoint_adjustment: RefloatSetpointAdjustment,
...
impl HapticFeedbackState {
    ...
    pub(super) fn update(
        &mut self,
        config: RefloatHapticConfig<'_>,
        input: HapticFeedbackInput,
        motor: &impl MotorOutput,
        motor_control: &mut RefloatMotorControl,
        now: TimestampTicks,
...
fn feedback_type(
    ...
    if !matches!(input.run_state, RefloatRunState::Running)
        || matches!(input.mode, RefloatMode::HandTest)
    {
    ...
    match input.setpoint_adjustment {
        ...
        RefloatSetpointAdjustment::PushbackSpeed => return HapticFeedbackType::DutySpeed,
        ...
        | RefloatSetpointAdjustment::PushbackError => return HapticFeedbackType::ErrorVoltage,
        _ => {}
    }
...
fn strength_scale(config: RefloatHapticConfig<'_>, speed: Speed) -> f32 {
...
mod tests {
    ...
    fn duty_input() -> HapticFeedbackInput {
        HapticFeedbackInput {
            ...
            setpoint_adjustment: RefloatSetpointAdjustment::PushbackDuty,
    ...
    #[test]
    fn duty_pushback_starts_the_scaled_warning_tone_like_refloat() {
        let firmware = FirmwareTest::new();
        let mut haptic = HapticFeedbackState::new();
        let mut motor_control = RefloatMotorControl::new();

        haptic.update(
            RefloatConfigImage::defaults().haptic(),
            duty_input(),
            firmware.motor(),
            &mut motor_control,
    ...
    #[test]
    fn generated_haptic_defaults_decode_at_the_refloat_offsets() {
        let defaults = RefloatConfigImage::defaults();
    ...
    fn handtest_stops_an_active_haptic_tone() {
        ...
        handtest.mode = RefloatMode::HandTest;
        haptic.update(
            config.haptic(),
            handtest,
            firmware.motor(),
    ...
    fn configured_current_saturation_starts_the_continuous_warning() {
        ...
        let mut motor_control = RefloatMotorControl::new();
        let mut input = duty_input();
        input.setpoint_adjustment = RefloatSetpointAdjustment::None;
