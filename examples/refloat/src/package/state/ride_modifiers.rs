...
use crate::domain::{
    RefloatRealtimeRuntimeSetpoint, RefloatRealtimeRuntimeSetpoints, RefloatWheelSlipState,
};
...
fn smooth_ramp(state: &mut SmoothAngle, target: AngleDegrees, step: AngleDegrees, smoothing: f32) {
    ...
    // `third_party/refloat/src/utils.c:36-64` with a 1.5 degree center window.
    state.target = target;
...
impl TurnTiltState {
    fn aggregate(&mut self, yaw: AngleDegrees) {
        ...
        // `third_party/refloat/src/turn_tilt.c:45-72` and
        // `third_party/refloat/src/main.c:800`.
...
pub(super) struct RideModifierInput {
    ...
    pub(super) wheelslip: RefloatWheelSlipState,
}
...
impl RideModifierState {
    ...
    pub(super) fn advance(
        ...
        if !input.darkride {
            if matches!(input.wheelslip, RefloatWheelSlipState::Detected) {
                // C map: wheelslip freezes nose angling and winds modifier state
                // down at `third_party/refloat/src/main.c:881-887`.
        ...
        RefloatRealtimeRuntimeSetpoints::new(
            RefloatRealtimeRuntimeSetpoint::new(board),
            RefloatRealtimeRuntimeSetpoint::new(self.atr.angle.setpoint),
            RefloatRealtimeRuntimeSetpoint::new(self.brake.setpoint),
            RefloatRealtimeRuntimeSetpoint::new(self.torque.setpoint),
            RefloatRealtimeRuntimeSetpoint::new(self.turn.angle.setpoint),
            RefloatRealtimeRuntimeSetpoint::new(input.remote_setpoint),
        )
    }

    fn update_nose(&mut self, config: &RefloatConfigImage, erpm: Rpm, sample_rate: SampleRate) {
        // C map: constant/variable nose target and rate limit mirror
        // `third_party/refloat/src/main.c:746-758` and configuration at `:165-173`.
    ...
    fn update_torque(
        &mut self,
        config: crate::config::RefloatBalanceConfig<'_>,
        current: Current,
        braking: bool,
        abs_erpm: f32,
        ...
        // `third_party/refloat/src/torque_tilt.c:44-82`.
    ...
    fn update_atr(
        ...
        // and ramp selection mirror `third_party/refloat/src/atr.c:52-171`.
    ...
    fn update_brake(
        ...
        // `third_party/refloat/src/brake_tilt.c:42-91`.
    ...
    fn update_turn(
        ...
        // `third_party/refloat/src/turn_tilt.c:74-130`.
...
mod tests {
    ...
    #[test]
    fn turn_tilt_uses_filtered_yaw_and_erpm_direction_like_refloat() {
    ...
    #[test]
    fn brake_tilt_uses_balance_offset_while_regenerating_like_refloat() {
        let mut config = RefloatConfigImage::defaults();
    ...
    #[test]
    fn wheelslip_winds_down_and_aggregates_the_stronger_matching_torque_like_refloat() {
        ...
        let setpoints = state.advance(
            ...
            RideModifierInput {
                wheelslip: RefloatWheelSlipState::Detected,
                ..input()
            },
        );
    ...
    #[test]
    fn darkride_keeps_remote_tilt_but_suppresses_ride_modifiers_like_refloat() {
        let mut state = RideModifierState {
