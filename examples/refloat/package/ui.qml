...
var pkgName="Refloat";var pkgVersion="1.2.1";if(pkgName.includes("PACKAGE_NAME")){return;}
...
var version=packageLoader.getPackageVersion(pkgName);if(!version){print("Refloat: Failed to retrieve package version");return;}
...
if(packageConfig.isDefault()){var backup=loadBackup("auto_backup_"+board.getId());if(backup){print("Refloat: Automatic backup restore.");applyConfigDialog.show(backup,true,false);}else{print("Refloat: No automatic backup to restore.");}}else{print("Refloat: Automatic config backup.");autoBackup();}}
...
Component.onCompleted:{if(typeof confCustomLoader!==typeof undefined){print("Refloat: Hooking custom config Write button...");customConfigWriteButton=locateButton(confCustomLoader.item,"Write");if(customConfigWriteButton){customConfigWriteButton.clicked.connect(packageConfig.configWritten);}else{print("Refloat: Failed to locate Write button.");}}
if(preferences.showWelcomeDialog){welcomeDialog.show();}
onStartupTimer.start();}
Component.onDestruction:{if(typeof confCustomLoader!==typeof undefined){print("Refloat: Unhooking custom config Write button...");if(customConfigWriteButton){customConfigWriteButton.clicked.disconnect(packageConfig.configWritten);}else{print("Refloat: Write button was not set.");}}}
...
ErrDesc{text:"Refloat package is disabled. After you're done with board setup, re-enable it."
visible:state.pkgState===state.s_Disabled}
Button{Layout.alignment:Qt.AlignHCenter
text:"Enable Refloat"
visible:state.pkgState===state.s_Disabled
...
ErrText{text:"Refloat Cfg failed to load"
