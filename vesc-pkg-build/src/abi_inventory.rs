...
mod tests {
    ...
    fn groups_requirements_by_abi_role() {
        ...
        assert!(abi.iter().any(|item| item.kind == AbiRequirementKind::EntryPoint));
        assert!(abi.iter().any(|item| item.kind == AbiRequirementKind::Macro));
        assert!(abi.iter().any(|item| item.kind == AbiRequirementKind::Type));
        assert!(abi.iter().any(|item| item.kind == AbiRequirementKind::Function));
        assert!(abi.iter().any(|item| item.kind == AbiRequirementKind::ErrorSymbol));
    }
}
